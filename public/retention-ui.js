import {CROPS,ITEMS,QUESTS,DAILY_REWARDS,DAY_MS,utcDay,dailyTasks,dailyOrders,levelOf,seedCost,levelProgress,SILO_COSTS,siloBonus} from './farm-state.js';
import {farmNow} from './farm-client.js';
const $=id=>document.getElementById(id);
const icons=()=>window.lucide?.createIcons();
export function createRetentionUI({state,runAction,onChange,notify,getCrop,itemList}){
 let tab='challenges',utility='tractor',lastDay=utcDay(farmNow()),lastTractorReady=true,lastFieldStatus='';
 ...
 function gift(){
  ...
  $('daily-gift').innerHTML=`<section class="gift-panel"><div class="gift-heading"><span class="gift-icon"><i data-lucide="gift"></i></span><div><h3>${claimed?'A little gift, just for you':'Good to see you, farmer!'}</h3><p>${streak?`${streak} day${streak===1?'':'s'} in a row · `:''}Best streak: ${state.login.best} days</p></div><span class="streak-pill"><i data-lucide="flame"></i> ${streak}</span></div><div class="streak-days">${DAILY_REWARDS.map((reward,i)=>`<div class="streak-day ${i===day?'current':''} ${i<day||claimed&&i===day?'collected':''}"><small>Day ${i+1}</small><i data-lucide="${i<day||claimed&&i===day?'check':i===6?'gift':'coins'}"></i><strong>${reward}</strong></div>`).join('')}</div><button id="checkin-gift" class="primary-button" ${claimed?'disabled':''}>${claimed?'Gift collected · see you tomorrow!':`Collect day ${day+1} gift · ${DAILY_REWARDS[day]} coins`}<i data-lucide="${claimed?'check':'gift'}"></i></button><small class="gift-note">A new seven-day cycle follows day 7. Miss a day? Start again with your farm intact.</small></section>`;
  $('checkin-gift').onclick=()=>act({type:'checkin'},r=>`Welcome back! +${r.coins} coins · ${r.streak}-day streak.`);
 }
 function renderToday(){
  ...
  if(tab==='challenges'){
   ...
   $('today-content').innerHTML=`<div class="daily-bonus"><i data-lucide="sparkles"></i><span>Finish all three for <strong>60 bonus coins + 15 XP</strong></span><b>${tasks.filter(q=>q.claimed).length}/3</b></div>`+tasks.map(q=>`<article class="daily-task ${q.claimed?'completed':''}"><div><h3>${q.title}</h3><p>${q.description}</p></div><span class="daily-reward">${q.reward} coins</span><progress max="${q.target}" value="${q.progress}" aria-label="${q.title} progress"></progress><div class="task-bottom"><span>${q.progress} / ${q.target} · +10 XP</span><button class="small-button" data-daily="${q.id}" ${q.claimed||q.progress<q.target?'disabled':''}>${q.claimed?'Collected':'Claim reward'}</button></div></article>`).join('');
   document.querySelectorAll('[data-daily]').forEach(b=>b.onclick=()=>act({type:'daily',id:Number(b.dataset.daily),day},r=>`Challenge complete! +${r.coins} coins${r.bonus?' including your daily bonus!':'.'}`));
  }else{
   $('today-content').innerHTML='<p class="section-copy">Fill the cart for your neighbours. Orders pay more than the market, and each can be delivered once today.</p>'+dailyOrders(state,farmNow()).map(o=>{const can=Object.entries(o.input).every(([k,n])=>state.inventory[k]>=n);return `<article class="order-card ${o.done?'completed':''}"><div class="order-heading"><span class="order-icon"><i data-lucide="${o.done?'circle-check':'truck'}"></i></span><div><h3>${o.title}</h3><small>${o.coins} coins · +${o.xp} XP</small></div></div><div class="ingredients">${itemList(o.input,true)}</div><button class="small-button" data-order="${o.id}" ${o.done||!can?'disabled':''}>${o.done?'Delivered':can?'Load cart & deliver':'Gather these ingredients'}</button></article>`;}).join('');
   document.querySelectorAll('[data-order]').forEach(b=>b.onclick=()=>act({type:'delivery',id:Number(b.dataset.order),day},r=>`Delivery complete! +${r.coins} coins and +${r.xp} XP.`));
  }
  countdown();icons();
 }
 ...
 function renderUtility(){
  if(utility==='tractor'){
   ...
   const empty=state.plots.filter(p=>!p.crop).length,plant=Math.min(empty,Math.floor(state.coins/seedCost(state,crop))),water=state.plots.filter(p=>p.crop&&!p.watered&&p.readyAt>farmNow()).length,harvest=state.plots.filter(p=>p.crop&&p.readyAt<=farmNow()).length;
   $('utility-content').innerHTML=`<div class="utility-hero"><i data-lucide="tractor"></i><div><h3>A helping hand in the fields</h3><p>Work every eligible field in one go. The tractor rests for 15 seconds after a job.</p></div></div><div class="tractor-crop">Selected crop: <strong>${CROPS[crop].name}</strong><span>${seedCost(state,crop)} coins per field</span></div><div class="tractor-actions">${[['plant','sprout',`Plant ${CROPS[crop].name}`,plant,`${plant*seedCost(state,crop)} coins`],['water','droplets','Water growing crops',water,'Free'],['harvest','shopping-basket','Harvest ready crops',harvest,'Free']].map(([mode,icon,label,n,cost])=>`<button data-tractor="${mode}" ${cooldown||!n?'disabled':''}><i data-lucide="${icon}"></i><span><strong>${label}</strong><small>${n} fields · ${cost}</small></span><i data-lucide="chevron-right"></i></button>`).join('')}</div><p id="tractor-timer" class="utility-note">${cooldown?`Ready again in ${cooldown}s`:'Your tractor is ready. Choose a job.'}</p>`;
   document.querySelectorAll('[data-tractor]').forEach(b=>b.onclick=()=>act({type:'tractor',mode:b.dataset.tractor,crop:getCrop()},r=>`All done! The tractor worked ${r.count} fields.`));
  }else{
