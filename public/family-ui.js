...
export function createFamilyUI({state,runAction,notify,isReady}){
 ...
 function landing(){
  ...
  return `${rewardCards()}<div class="family-welcome">${art('family-members')}<h3>A little farm. A bigger family.</h3><p>Share a weekly order, help each other grow and join the Family Tournament. Up to ${view.config.maxMembers} farmers can play together.</p></div>${cooldown?`<p class="family-notice">You can join or create a family in ${formatDuration(view.cooldownUntil-farmNow())}.</p>`:''}<div class="family-join-grid"><form data-family-form="create"><h3>Create a family</h3><label for="family-name">Family name</label><input id="family-name" name="name" required minlength="3" maxlength="20" placeholder="Meadow friends" autocomplete="off"><fieldset><legend>Choose your emblem</legend><div class="family-emblems">${FAMILY_EMBLEMS.map((e,i)=>`<label title="${esc(e.icon)}"><input type="radio" name="emblem" value="${e.id}" ${i===0?'checked':''} aria-label="${esc(e.icon)} emblem">${emblem(e.id)}</label>`).join('')}</div></fieldset><button class="primary-button" ${disabled(cooldown)}>Create family</button></form><form data-family-form="join"><h3>Join with a code</h3><p>Ask a friend for their family’s invite code.</p><label for="family-code">Invite code</label><input id="family-code" name="code" required minlength="6" maxlength="6" placeholder="ABC234" autocapitalize="characters" autocomplete="off" spellcheck="false"><button class="primary-button" ${disabled(cooldown)}>Join family</button><small>You can belong to one family at a time. Leaving starts a 48-hour join cooldown.</small></form></div><section><h3>Open families</h3>${view.openFamilies.length?view.openFamilies.map(f=>`<div class="family-list-row">${emblem(f.emblem)}<div><strong>${esc(f.name)}</strong><span>${f.members} / ${view.config.maxMembers} farmers</span></div>${actionButton('family_join','Join',`data-family-id="${f.id}"`,cooldown)}</div>`).join(''):'<p>No open families yet. Create one or join with a friend’s code.</p>'}</section>`;
 }
 ...
 function week(){
  ...
  return `${rewardCards()}<section class="family-week-intro"><div><span class="eyebrow">${o.completed?'ORDER COMPLETE':'GROW SOMETHING TOGETHER'}</span><h3>This week’s Family Order</h3><p>Ends in <strong data-family-countdown>${formatDuration(view.endsAt-farmNow())}</strong> · Monday, 00:00 UTC</p></div>${art('family-weekly-order')}</section><p class="family-notice">${locked?'You have already contributed to another family this week. You can help this family next week.':`Deliver goods together. Every delivery also earns tournament points. Delivered goods cannot be taken back.`}</p>${prizePreview()}<div class="family-order">${lines.map(([key,target])=>{const filled=o.filled[key]??0,stock=state.inventory[key]??0,max=Math.min(stock,target-filled);return `<article class="family-order-line">${art(key)}<div class="family-line-copy"><strong>${ITEMS[key].name}</strong><span>${num(filled)} / ${num(target)} delivered · ${num(stock)} in stock</span><progress max="${target}" value="${filled}" aria-label="${ITEMS[key].name} delivered"></progress></div><div class="family-line-actions">${filled>=target?'<span class="family-done">✓ Complete</span>':[1,5,'max'].map(n=>actionButton('family_contribute',n==='max'?`Max (${num(max)})`:`+${n}`,`data-item="${key}" data-count="${n==='max'?max:n}"`,locked||max<(n==='max'?1:n))).join('')}</div></article>`;}).join('')}</div><section class="family-your-share"><h3>${o.completed?'Order complete':'Finish the order for extra rewards'}</h3><p>${view.yourOrderPoints>=view.config.minPoints?`${num(view.rewardPreview.coins)} coins · ${num(view.rewardPreview.xp)} XP · ${view.rewardPreview.diamonds} diamonds + a share of the completion bonus`:`${num(Math.max(0,view.config.minPoints-view.yourOrderPoints))} more order points to qualify for coins, XP and bonus diamonds.`}</p><small>Your contribution: ${num(view.yourPoints)} points${o.completed?' · Earned rewards appear above.':' · Rewards unlock when all four goods are delivered.'}</small></section><details class="family-extra"><summary>Tournament goods <span>${extrasOpen?'Extra points for your family':'Fill an order line to unlock'}</span></summary><p>Extra goods earn tournament points only. ${num(view.config.extraCap-view.extraUsed)} points left in your weekly allowance. They are handed in permanently.</p>${extrasOpen&&available.length?`<form data-family-form="extra"><label for="family-extra-item">Goods to contribute</label><select id="family-extra-item" name="item">${available.map(([key,item])=>`<option value="${key}">${esc(item.name)} · ${num(state.inventory[key])} in stock · ${num(item.sell)} points each</option>`).join('')}</select><label for="family-extra-count">Quantity</label><input id="family-extra-count" name="count" type="number" inputmode="numeric" min="1" step="1" value="1" required><button class="small-button" ${disabled(locked)}>Contribute goods</button></form>`:extrasOpen?'<p>No spare goods in storage yet.</p>':''}</details>`;
 }
 ...
 function render(){
  ...
  content.querySelectorAll('[data-family-action]').forEach(b=>b.onclick=()=>{
   ...
   if(['family_leave','family_kick','family_promote','family_code'].includes(type)&&!confirm({family_leave:'Leave this family? Joining another family will be unavailable for 48 hours.',family_kick:'Remove this member? A 48-hour join cooldown will apply.',family_promote:'Give this member leadership? You will become a regular member.',family_code:'Replace the invite code? The old code will stop working.'}[type]))return;
   act({type,week:view.week,item:b.dataset.item,count:Number(b.dataset.count),memberId:b.dataset.memberId,rewardId:b.dataset.rewardId,familyId:b.dataset.familyId,open:b.dataset.open==='true'});
  });
  ...
  content.querySelector('#family-copy')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(view.family.inviteCode);notify('Invite code copied.');}catch{notify('Select the invite code to copy it.');}});
 ...
 function refresh(){
  ...
  dot.hidden=!(view?.rewards.length||view?.order&&!view.contributionLocked&&Object.entries(view.order.lines).some(([k,n])=>(view.order.filled[k]??0)<n&&(state.inventory[k]??0)>0));
