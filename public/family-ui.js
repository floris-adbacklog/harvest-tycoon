...
export function createFamilyUI({state,runAction,notify,isReady}){
 // Daily sharing opens from the Members tab and comes back to it; it borrows this view's portraits and levels.
 const social=createSocialUI({state,notify,refreshFarm:()=>window.harvestRefresh(),getMembers:()=>view?.members??[],onBack:()=>{tab='members';open();}});
 ...
 function render(){
  ...
  content.querySelector('[data-social-open]')?.addEventListener('click',()=>social.open());
 ...
 async function load(force=false){
  ...
  try{const data=await window.parent.harvestBridge.request({operation:'family'});if(ticket!==generation)return;view=data.family;lastRead=Date.now();error='';
   ...
   if(!content.contains(document.activeElement)||!(['INPUT','SELECT'].includes(document.activeElement.tagName)||document.activeElement.closest('[data-emblem-picker]')))render();
