import {rookieLeft,ROOKIE_MS,ROOKIE_TIMER_BOOST,CROPS} from './farm-state.js';
...
export const rookieLabel=ms=>`${percent}% shorter waiting · ${rookieTimeLeft(ms)} left`;
...
export function createRookieUI({state,document:doc=globalThis.document,now=farmNow}){
 ...
 function render(left){
  ...
  dialog.innerHTML=`<button type="button" class="rookie-close" data-rookie-close aria-label="Close">×</button>${art('hourglass','rookie-art')}<p class="eyebrow">BEGINNER BOOST</p>`+
   (on?`<h2 id="rookie-title">${percent}% shorter waiting times</h2><p>Your first ${ROOKIE_MS/60000} minutes are a sprint: new crops and batches finish in a fraction of the time.</p>
    ...
    <li><strong>Your starter goods</strong><span>Your starter corn and animal feed are kept for your first steps. When the boost ends they are yours to sell.</span></li></ul>`
   :`<h2 id="rookie-title">Beginner boost ended</h2><p>Your first ${ROOKIE_MS/60000} minutes are over. Waiting times are back to normal, and your starter corn and animal feed are free to sell.</p>`)+
 ...
 function open(){
  ...
  render(rookieLeft(state,now()));if(!dialog.open)dialog.showModal();
 ...
 function tick(){
  const left=rookieLeft(state,now());
  const button=doc.getElementById('rookie-button'),badge=doc.getElementById('rookie-time');
  if(button){button.hidden=left<=0;button.title=left>0?`Beginner boost · ${rookieLabel(left)}`:'Beginner boost';}
  ...
  else if(left>0){dialog.querySelector('[data-rookie-left]').textContent=`${rookieTimeLeft(left)} left`;dialog.querySelector('progress').value=left;}
 }
 tick();
