...
export function fieldPicker({id,plots,selected=[],multiple=false,now,disabled=false,available=0,hint='Choose crops to give a head start',perItem='1 fertilizer per field',picture='fertilizer'}){
 ...
 }).join('')}</div>${multiple?`<div class="field-picker-bottom"><span>${perItem}</span><button type="button" data-picker-done>Done</button></div>`:''}</div></details>`;
}
...
export function bindFieldPicker(root,{multiple=false,available=0,noun=['field','fields'],onChange}){
 ...
 const update=()=>{const ids=selection();if(multiple)root.querySelector('[data-picker-title]').textContent=`${ids.length} ${ids.length===1?noun[0]:noun[1]} selected`;onChange(ids);};
