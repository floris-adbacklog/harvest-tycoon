...
// Why a farmer cannot be invited right now, or '' when they can: the same answer in the invite search and on a profile.
export function inviteBlocker(view,p,playerId){
 if(p.playerId===playerId)return 'You';if(p.family)return 'Already in a family';if(p.level<view.config.minLevel)return `Level ${view.config.minLevel} required`;
 if(view.sentInvitations?.some(i=>i.recipientId===p.playerId))return 'Invited';if(view.family.members>=view.config.maxMembers)return 'Family full';return '';
}
export function createFamilyInviteSearch({request,onInvite,getView,playerId,isBusy}){
 ...
 const candidate=p=>inviteBlocker(getView(),p,playerId);
 ...
 return {
  html(){if(familyId!==getView().family?.id){familyId=getView().family?.id;query='';players=[];status='Enter at least 2 characters to find a farmer.';}return `<section class="family-name-invite"><div class="family-invite-head">${art('family-members')}<div><h3>Invite a farmer</h3><p>They can accept or decline in their Family Hall.</p></div></div><label for="family-invite-name" class="family-invite-label">Player name</label><input id="family-invite-name" type="search" maxlength="20" autocomplete="off" placeholder="Search player name…" value="${esc(query)}" aria-describedby="family-invite-search-status"><p id="family-invite-search-status" data-invite-search-status role="status">${esc(status)}</p><div data-invite-search-results>${results()}</div><small>From level ${getView().config.minLevel} · one open invitation per farmer · 7 days</small></section>`;},
