...
// on top of it, live: a gold ring where something is ready, and a white frame for what is on screen now. Click or drag on it to
...
export function createMinimap({root,background,project,unproject,points,view,onJump}){
 ...
 function draw(){
  ...
  for(const p of points()){
   ...
   ctx.beginPath();ctx.arc(x,y,(pulse?7:6)*dpr,0,Math.PI*2);ctx.lineWidth=2.4*dpr;ctx.strokeStyle='#f5c542';ctx.stroke();
   ctx.beginPath();ctx.arc(x,y,(pulse?9.5:8.5)*dpr,0,Math.PI*2);ctx.lineWidth=1.4*dpr;ctx.strokeStyle='rgba(245,197,66,.45)';ctx.stroke();
  }
