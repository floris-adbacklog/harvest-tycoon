//#region ../validation/node_modules/@supabase/supabase-js/dist/tracingRegistry.mjs
var e = Symbol.for("@supabase/supabase-js.traceContextExtractor");
function t() {
	return globalThis[e];
}
//#endregion
//#region ../validation/node_modules/tslib/tslib.es6.mjs
function n(e, t) {
	var n = {};
...
//#endregion
//#region ../validation/node_modules/@supabase/functions-js/dist/module/helper.js
var i = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), a = class extends Error {
...
//#endregion
//#region ../validation/node_modules/@supabase/functions-js/dist/module/FunctionsClient.js
var u = class {
	constructor(e, { headers: t = {}, customFetch: n, region: r = l.Any } = {}) {
		this.url = e, this.headers = t, this.region = r, this.fetch = i(n);
	}
	setAuth(e) {
	...
	invoke(e) {
		return r(this, arguments, void 0, function* (e, t = {}) {
			...
			let r, i, a;
			try {
				let { headers: n, method: l, body: u, signal: d, timeout: f } = t, p = {}, { region: m } = t;
				m ||= this.region;
				...
				let _ = d;
				f && (i = new AbortController(), r = setTimeout(() => i.abort(), f), d ? (_ = i.signal, a = () => i.abort(), d.addEventListener("abort", a)) : _ = i.signal);
				let v = yield this.fetch(h.toString(), {
					method: l || "POST",
				...
				if (y && y === "true") throw new s(v);
				if (!v.ok) throw new c(v);
				let b = (v.headers.get("Content-Type") ?? "text/plain").split(";")[0].trim().toLowerCase(), te;
				return te = b === "application/json" ? yield v.json() : b === "application/octet-stream" || b === "application/pdf" ? yield v.blob() : b === "text/event-stream" ? v : b === "multipart/form-data" ? yield v.formData() : yield v.text(), {
					data: te,
					error: null,
					response: v
				};
				...
				return {
					data: null,
					error: e,
					response: e instanceof c || e instanceof s ? e.context : void 0
				};
			} finally {
...
async function re(e, t, n, r) {
	...
	for (;;) {
		...
		try {
			...
			if (e?.name === "AbortError" || e?.code === "ABORT_ERR" || !h.includes(n.method)) throw e;
			if (r && i < f) {
				let e = p(i);
				i++, await te(e, n.signal);
				continue;
			}
			throw e;
		}
		if (ne(n.method, o.status, i, r)) {
			let e = o.headers?.get("Retry-After") ?? null, t = e === null ? p(i) : Math.max(0, parseInt(e, 10) || 0) * 1e3;
			await o.text(), i++, await te(t, n.signal);
			continue;
		}
		return o;
	}
...
var ie = class {
	...
	then(e, t) {
		...
		let r = this.fetch, i = (async () => {
			...
			let t = await re(r, n.url.toString(), {
				method: n.method,
				headers: e,
		...
		if (e.ok) {
			if (t.method !== "HEAD") {
				...
				if (i !== "") if (t.headers.get("Accept") === "text/csv") r = i;
					...
					if (n = { message: i }, r = null, t.shouldThrowOnError) throw new d({
						message: i,
						details: "",
						hint: "",
						code: ""
			...
			if (s && c && c.length > 1 && (i = parseInt(c[1])), t.isMaybeSingle && Array.isArray(r)) if (r.length > 1) {
				...
				}, r = null, i = null, a = 406, o = "Not Acceptable", t.shouldThrowOnError) throw new d(b(b({}, n), {}, { hint: n.hint ?? "" }));
			...
			if (n && t.shouldThrowOnError) throw new d(n);
		}
		return {
			success: n === null,
			error: n,
...
}, oe = /* @__PURE__ */ RegExp("[,()]"), x = class extends ae {
	throwOnError() {
		return super.throwOnError();
	}
	eq(e, t) {
	...
	in(e, t) {
		let n = Array.from(new Set(t)).map((e) => typeof e == "string" && oe.test(e) ? `"${e}"` : `${e}`).join(",");
	...
	select(e, t) {
		...
		return s.searchParams.set("select", o), r && c.append("Prefer", `count=${r}`), new x({
			method: i,
			url: s,
			headers: c,
			schema: this.schema,
	...
	update(e, { count: t } = {}) {
		...
		return t && r.append("Prefer", `count=${t}`), new x({
			method: "PATCH",
			url: n,
			headers: r,
			schema: this.schema,
	...
	delete({ count: e } = {}) {
		...
		return e && n.append("Prefer", `count=${e}`), new x({
			method: "DELETE",
			url: t,
			headers: n,
			schema: this.schema,
...
function ce(e, t) {
	try {
		let t = JSON.parse(e);
		if (t && typeof t == "object" && !Array.isArray(t)) return new d({
...
var ue = class e {
	constructor(e, { headers: t = {}, schema: n, fetch: r, timeout: i, urlLengthLimit: a = 8e3, retry: o } = {}) {
		...
		i !== void 0 && i > 0 ? this.fetch = (e, t) => {
			...
			if (a) {
				...
				return a.addEventListener("abort", i, { once: !0 }), s(e, b(b({}, t), {}, { signal: n.signal })).finally(() => {
					clearTimeout(r), a.removeEventListener("abort", i);
				});
			}
			return s(e, b(b({}, t), {}, { signal: n.signal })).finally(() => clearTimeout(r));
	...
	async getOpenApiSpec() {
		...
		try {
			...
			return le(e, i.status, i.statusText);
		}
		if (i.ok) try {
			return {
				success: !0,
	...
	rpc(e, t = {}, { head: n = !1, get: r = !1, count: i } = {}) {
		...
		return l ? u.set("Prefer", i ? `count=${i},return=minimal` : "return=minimal") : i && u.set("Prefer", `count=${i}`), new x({
			method: a,
			url: o,
			headers: u,
			schema: this.schemaName,
...
}, fe = "realtime-js/2.116.0", pe = "1.0.0", me = "2.0.0", he = me, ge = 1e4, _e = 15e3, ve = 1e4, S = {
	closed: "closed",
	errored: "errored",
	joined: "joined",
	joining: "joining",
...
var Se = (e, t, n = {}) => {
	let r = n.skipTypes ?? [];
	return t ? Object.keys(t).reduce((n, i) => (n[i] = Ce(i, e, t, r), n), {}) : {};
}, Ce = (e, t, n, r) => {
	let i = t.find((t) => t.name === e)?.type, a = n[e];
	return i && !r.includes(i) ? we(i, a) : Te(a);
}, we = (e, t) => {
	if (e.charAt(0) === "_") return ke(t, e.slice(1, e.length));
	switch (e) {
		case C.bool: return Ee(t);
		case C.float4:
		case C.float8:
		case C.int2:
		case C.int4:
		case C.int8:
		case C.numeric:
		case C.oid: return De(t);
		case C.json:
		case C.jsonb: return Oe(t);
		case C.timestamp: return Ae(t);
		case C.abstime:
		case C.date:
		case C.daterange:
		case C.int4range:
	...
	if (e[0] === "{" && r === "}") {
		...
		return r.map((e) => we(t, e));
	}
	return e;
}, Ae = (e) => typeof e == "string" ? e.replace(" ", "T") : e, je = (e) => {
	let t = new URL(e);
	...
	return e;
}, Ne = typeof self < "u" ? self : null, Pe = typeof window < "u" ? window : null, w = Ne || Pe || globalThis, Fe = "2.0.0", Ie = 1e4, Le = 1e3, Re = 100, T = {
	connecting: 0,
	open: 1,
	closing: 2,
	closed: 3
	...
	constructor(e, t, n) {
		this.state = E.closed, this.topic = e, this.params = Me(t || {}), this.socket = n, this.bindings = [], this.bindingRef = 0, this.timeout = this.socket.timeout, this.joinedOnce = !1, this.joinPush = new He(this, D.join, this.params, this.timeout), this.pushBuffer = [], this.stateChangeRefs = [], this.rejoinTimer = new Ue(() => {
			...
			this.state = E.joined, this.rejoinTimer.reset(), this.pushBuffer.forEach((e) => e.send()), this.pushBuffer = [];
		}), this.joinPush.receive("error", (e) => {
			this.state = E.errored, this.socket.hasLogger() && this.socket.log("channel", `error ${this.topic}`, e), this.socket.isConnected() && this.rejoinTimer.scheduleTimeout();
		}), this.onClose(() => {
			this.rejoinTimer.reset(), this.socket.hasLogger() && this.socket.log("channel", `close ${this.topic}`), this.state = E.closed, this.socket.remove(this);
		}), this.onError((e) => {
			this.socket.hasLogger() && this.socket.log("channel", `error ${this.topic}`, e), this.isJoining() && this.joinPush.reset(), this.state = E.errored, this.socket.isConnected() && this.rejoinTimer.scheduleTimeout();
		}), this.joinPush.receive("timeout", () => {
			this.socket.hasLogger() && this.socket.log("channel", `timeout ${this.topic}`, this.joinPush.timeout), new He(this, D.leave, Me({}), this.timeout).send(), this.state = E.errored, this.joinPush.reset(), this.socket.isConnected() && this.rejoinTimer.scheduleTimeout();
		}), this.on(D.reply, (e, t) => {
	...
	teardown() {
		this.pushBuffer.forEach((e) => e.destroy()), this.pushBuffer = [], this.rejoinTimer.reset(), this.joinPush.destroy(), this.state = E.closed, this.bindings = [];
	}
	onClose(e) {
		this.on(D.close, e);
	}
	...
	leave(e = this.timeout) {
		this.rejoinTimer.reset(), this.joinPush.cancelTimeout(), this.state = E.leaving;
		let t = () => {
			this.socket.hasLogger() && this.socket.log("channel", `leave ${this.topic}`), this.trigger(D.close, "leave");
		}, n = new He(this, D.leave, Me({}), e);
	...
	rejoin(e = this.timeout) {
		this.isLeaving() || (this.socket.leaveOpenTopic(this.topic), this.state = E.joining, this.joinPush.resend(e));
	}
	trigger(e, t, n, r) {
	...
	static request(e, t, n, r, i, a, o) {
		if (w.XDomainRequest) {
			let n = new w.XDomainRequest();
			return this.xdomainRequest(n, e, t, r, i, a, o);
		} else if (w.XMLHttpRequest) {
			let s = new w.XMLHttpRequest();
			return this.xhrRequest(s, e, t, n, r, i, a, o);
		} else if (w.fetch && w.AbortController) return this.fetchRequest(e, t, n, r, i, a, o);
	...
	static fetchRequest(e, t, n, r, i, a, o) {
		...
		return i && (c = new AbortController(), setTimeout(() => c.abort(), i), s.signal = c.signal), w.fetch(t, s).then((e) => e.text()).then((e) => this.parseJSON(e)).then((e) => o && o(e)).catch((e) => {
	...
	closeAndRetry(e, t, n) {
		this.close(e, t, n), this.readyState = T.connecting;
	}
	ontimeout() {
	...
	constructor(e, t = {}) {
		this.stateChangeCallbacks = {
			...
			error: [],
			message: []
		}, this.channels = [], this.sendBuffer = [], this.ref = 0, this.fallbackRef = null, this.timeout = t.timeout || Ie, this.transport = t.transport || w.WebSocket || qe, this.conn = void 0, this.primaryPassedHealthCheck = !1, this.longPollFallbackMs = t.longPollFallbackMs, this.fallbackTimer = null;
		let n = null;
		try {
			n = w && w.sessionStorage;
		} catch {}
		...
		let r = null;
		Pe && Pe.addEventListener && (Pe.addEventListener("pagehide", (e) => {
			this.conn && (this.disconnect(), r = this.connectClock);
		}), Pe.addEventListener("pageshow", (e) => {
			r === this.connectClock && (r = null, this.connect());
		}), Pe.addEventListener("visibilitychange", () => {
			...
			console.log(`${e}: ${t}`, n);
		}), this.longpollerTimeout = t.longpollerTimeout || 2e4, this.params = Me(t.params || {}), this.endPoint = `${e}/${ze.websocket}`, this.vsn = t.vsn || Fe, this.heartbeatTimeoutTimer = null, this.heartbeatTimer = null, this.heartbeatSentAt = null, this.pendingHeartbeatRef = null, this.reconnectTimer = new Ue(() => {
			if (this.pageHidden) {
		...
		}, this.reconnectAfterMs), this.authToken = t.authToken && Me(t.authToken);
	}
	...
	connect(e) {
		e && (console && console.log("passing params to connect is deprecated. Instead pass :params to the Socket constructor"), this.params = Me(e)), !(this.conn && !this.disconnecting) && (this.longPollFallbackMs && this.transport !== qe ? this.connectWithFallback(qe, this.longPollFallbackMs) : this.transportConnect());
	}
	log(e, t, n) {
	...
	heartbeatTimeout() {
		if (this.pendingHeartbeatRef) {
			...
			this.triggerChanError(/* @__PURE__ */ Error("heartbeat timeout")), this.closeWasClean = !1, this.teardown(() => this.reconnectTimer.scheduleTimeout(), Le, "heartbeat timeout");
		}
	}
	resetHeartbeat() {
	...
	triggerChanError(e) {
		this.channels.forEach((t) => {
			t.isErrored() || t.isLeaving() || t.isClosed() || t.trigger(D.error, e);
		});
	}
	connectionState() {
		switch (this.conn && this.conn.readyState) {
			case T.connecting: return "connecting";
			case T.open: return "open";
			case T.closing: return "closing";
...
//#endregion
//#region ../validation/node_modules/@supabase/realtime-js/dist/module/RealtimePresence.js
var nt;
(function(e) {
...
//#endregion
//#region ../validation/node_modules/@supabase/realtime-js/dist/module/lib/normalizeChannelError.js
function it(e) {
...
//#endregion
//#region ../validation/node_modules/@supabase/realtime-js/dist/module/phoenix/channelAdapter.js
var at = class {
	constructor(e, t, n) {
		let r = ot(n);
...
//#endregion
//#region ../validation/node_modules/@supabase/realtime-js/dist/module/RealtimePostgresFilterBuilder.js
...
var gt = class e {
	...
	constructor(e, t = { config: {} }, n) {
		...
		}), this._updateFilterTransform(), this.broadcastEndpointURL = je(this.socket.socketAdapter.endPointURL()), this.private = this.params.config.private || !1, !this.private && this.params.config?.broadcast?.replay) throw Error(`tried to use replay on public channel '${this.topic}'. It must be a private channel.`);
	}
	subscribe(e, t = this.timeout) {
		if (this.socket.isConnected() || this.socket.connect(), this.channelAdapter.isClosed()) {
			let { config: { broadcast: n, presence: r, private: i, postgres_changes_options: a } } = this.params, o = this.bindings.postgres_changes?.map((e) => e.filter) ?? [], s = !!this.bindings[ht.PRESENCE] && this.bindings[ht.PRESENCE].length > 0 || this.params.config.presence?.enabled === !0, c = {}, l = Object.assign({
				broadcast: n,
			...
			this.socket.accessTokenValue && (c.access_token = this.socket.accessTokenValue), this._onError((t) => {
				e?.(O.CHANNEL_ERROR, it(t));
			}), this._onClose(() => e?.(O.CLOSED)), this.updateJoinPayload(Object.assign({ config: l }, c)), this._updateFilterMessage();
			let u = a?.wait && o.length > 0 ? Math.max(t, (a.timeout ?? _e) + ve) : t;
			this.channelAdapter.subscribe(u).receive("ok", async ({ postgres_changes: t }) => {
				...
				e?.(O.CHANNEL_ERROR, Error(n, { cause: t }));
			}).receive("timeout", () => {
				e?.(O.TIMED_OUT);
			});
		}
	...
	_updatePostgresBindings(t, n) {
		...
		for (let o = 0; o < i; o++) {
			...
			if (d && d.event === s && e.isFilterValueEqual(d.schema, c) && e.isFilterValueEqual(d.table, l) && e.isFilterValueEqual(d.filter, u)) a.push(Object.assign(Object.assign({}, i), { id: d.id }));
				...
				this.unsubscribe(), this.state = S.errored, n?.(O.CHANNEL_ERROR, /* @__PURE__ */ Error("mismatch between server and client bindings for postgres changes"));
				return;
			}
		}
		this.bindings.postgres_changes = a, this.state != S.errored && n && n(O.SUBSCRIBED);
	}
	presenceState() {
	...
	_notThisChannelEvent(e, t) {
		let { close: n, error: r, leave: i, join: a } = ye;
		return t && [
			n,
			r,
			i,
	...
	_getPayloadRecords(e) {
		...
		return (e.type === "INSERT" || e.type === "UPDATE") && (t.new = Se(e.columns, e.record)), (e.type === "UPDATE" || e.type === "DELETE") && (t.old = Se(e.columns, e.old_record)), t;
	}
}, _t = class {
	constructor(e, t) {
	...
	isConnecting() {
		return this.socket.connectionState() == be.connecting;
	}
	isDisconnecting() {
		return this.socket.connectionState() == be.closing;
	}
	connectionState() {
...
var Ct = "\n  addEventListener(\"message\", (e) => {\n    if (e.data.event === \"start\") {\n      setInterval(() => postMessage({ event: \"keepAlive\" }), e.data.interval);\n    }\n  });", wt = class {
	...
	constructor(e, t) {
		if (this.channels = [], this.accessTokenValue = null, this.accessToken = null, this.apiKey = null, this.httpEndpoint = "", this.headers = {}, this.params = {}, this.ref = 0, this.serializer = new xe(), this._manuallySetToken = !1, this._authPromise = null, this._authGeneration = 0, this._workerHeartbeatTimer = void 0, this._pendingWorkerHeartbeatRef = null, this._pendingDisconnectTimer = null, this._disconnectOnEmptyChannelsAfterMs = 0, this._resolveFetch = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), !t?.params?.apikey) throw Error("API key is required to connect to Realtime");
		this.apiKey = t.params.apikey;
		let n = this._initializeOptions(t);
		this.socketAdapter = new _t(e, n), this.httpEndpoint = je(e), this.fetch = this._resolveFetch(t?.fetch);
	}
	connect() {
	...
	connectionState() {
		return this.socketAdapter.connectionState() || be.closed;
	}
	isConnected() {
	...
	channel(e, t = { config: {} }) {
		...
		if (r) return r;
		{
			let n = new gt(`realtime:${e}`, t, this);
	...
	async _performAuth(e, t) {
		...
		t === this._authGeneration && (this.accessToken ? this._manuallySetToken = !1 : r && (this._manuallySetToken = !0), this.accessTokenValue != n && (this.accessTokenValue = n, this.channels.forEach((e) => {
			...
			e.updateJoinPayload(t), e.joinedOnce && e.channelAdapter.isJoined() && e.channelAdapter.push(ye.access_token, { access_token: n });
		})));
	}
	...
	_workerObjectUrl(e) {
		...
		if (e) t = e;
			...
			let e = new Blob([Ct], { type: "application/javascript" });
	...
	_initializeOptions(e) {
		...
		let t = {};
		t.timeout = e?.timeout ?? ge, t.heartbeatIntervalMs = e?.heartbeatIntervalMs ?? vt.HEARTBEAT_INTERVAL, this._disconnectOnEmptyChannelsAfterMs = e?.disconnectOnEmptyChannelsAfterMs ?? 2 * (e?.heartbeatIntervalMs ?? vt.HEARTBEAT_INTERVAL), t.transport = e?.transport ?? de.getWebSocketConstructor(), t.params = e?.params, t.logger = e?.logger, t.heartbeatCallback = this._wrapHeartbeatCallback(e?.heartbeatCallback), t.sessionStorage = e?.sessionStorage ?? St(), t.reconnectAfterMs = e?.reconnectAfterMs ?? ((e) => yt[e - 1] || bt);
		let n, r, i = e?.vsn ?? he;
		switch (i) {
			case pe:
...
function Ot(e) {
	...
	return { async request({ method: n, path: r, query: i, body: a, headers: o }) {
		let s = Et(e.baseUrl, r, i), c = await Dt(e.auth), l = await t(s, {
			method: n,
			headers: {
		...
		if (!l.ok) {
			...
			throw new Tt(t?.message ?? `Request failed with status ${l.status}`, {
				status: l.status,
				icebergType: t?.type,
				icebergCode: t?.code,
				details: e
...
var At = class {
	...
	async listNamespaces(e) {
		let t = e ? { parent: kt(e.namespace) } : void 0;
	...
	async dropNamespace(e) {
		await this.client.request({
			method: "DELETE",
			path: `${this.prefix}/namespaces/${kt(e.namespace)}`
		});
	}
	...
	async namespaceExists(e) {
		try {
			...
			if (e instanceof Tt && e.status === 404) return !1;
			throw e;
		}
	}
	async createNamespaceIfNotExists(e, t) {
		try {
			...
			if (e instanceof Tt && e.status === 409) return;
			throw e;
		}
	}
};
...
var Mt = class {
	...
	async createTableIfNotExists(e, t) {
		try {
			...
			if (n instanceof Tt && n.status === 409) return await this.loadTable({
				namespace: e.namespace,
				name: t.name
			});
			throw n;
	...
	constructor(e) {
		...
		this.client = Ot({
			...
			auth: e.auth,
			fetchImpl: e.fetch
		}), this.accessDelegation = e.accessDelegation?.join(","), this.namespaceOps = new At(this.client, t), this.tableOps = new Mt(this.client, t, this.accessDelegation);
	}
	async listNamespaces(e) {
...
//#endregion
//#region ../validation/node_modules/@supabase/storage-js/dist/index.mjs
function Pt(e) {
	"@babel/helpers - typeof";
...
var Gt = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), Kt = (e) => {
	if (typeof e != "object" || !e) return !1;
	let t = Object.getPrototypeOf(e);
	return (t === null || t === Object.prototype || Object.getPrototypeOf(t) === null) && !(Symbol.toStringTag in e) && !(Symbol.iterator in e);
}, qt = (e) => {
	if (Array.isArray(e)) return e.map((e) => qt(e));
...
}, Jt = (e) => !e || typeof e != "string" || e.length === 0 || e.length > 100 || e.trim() !== e || e.includes("/") || e.includes("\\") ? !1 : /^[\w!.\*'() &$@=;:+,?-]+$/.test(e), Yt = (e) => e.split("/").map(encodeURIComponent).join("/"), Xt = (e) => {
	...
	if (typeof e == "object" && e && "json" in e && typeof e.json == "function") {
		...
		Number.isFinite(i) || (i = 500), n.json().then((e) => {
			...
			t(new Vt(Xt(e), i, n, r, e?.code));
		}).catch(() => {
			let e = i + "";
			t(new Vt(n.statusText || `HTTP ${i} error`, i, e, r));
		});
	} else t(new Ht(Xt(e), e, r));
}, Qt = (e, t, n, r) => {
	let i = {
		method: e,
		headers: t?.headers || {}
	};
	if (e === "GET" || e === "HEAD" || !r) return k(k({}, i), n);
	if (Kt(r)) {
		let e = t?.headers || {}, n;
		for (let [t, r] of Object.entries(e)) t.toLowerCase() === "content-type" && (n = r);
		i.headers = Ut(e, "Content-Type", n ?? "application/json"), i.body = JSON.stringify(r);
	} else i.body = r;
	return t?.duplex && (i.duplex = t.duplex), k(k({}, i), n);
};
async function $t(e, t, n, r, i, a, o) {
	return new Promise((s, c) => {
		e(n, Qt(t, r, i, a)).then((e) => {
			...
			return e.json();
		}).then((e) => s(e)).catch((e) => Zt(e, c, r, o));
	});
}
function en(e = "storage") {
	return {
		get: async (t, n, r, i) => $t(t, "GET", n, r, i, void 0, e),
		post: async (t, n, r, i, a) => $t(t, "POST", n, i, a, r, e),
		put: async (t, n, r, i, a) => $t(t, "PUT", n, i, a, r, e),
		head: async (t, n, r, i) => $t(t, "HEAD", n, k(k({}, r), {}, { noResolveJson: !0 }), i, void 0, e),
		remove: async (t, n, r, i, a) => $t(t, "DELETE", n, i, a, r, e)
	};
}
var { get: A, post: j, put: tn, head: nn, remove: M } = en("storage"), N = en("vectors"), rn = class {
	constructor(e, t = {}, n, r = "storage") {
		this.shouldThrowOnError = !1, this.url = e, this.headers = Wt(t), this.fetch = Gt(n), this.namespace = r;
	}
	throwOnError() {
		return this.shouldThrowOnError = !0, this;
	}
	setHeader(e, t) {
		return this.headers = Ut(this.headers, e, t), this;
	}
	...
	async uploadOrUpdate(e, t, n, r) {
		...
		return i.handleOperation(async () => {
			let a, o = k(k({}, un), r), s = k(k({}, i.headers), e === "POST" && { "x-upsert": String(o.upsert) }), c = o.metadata;
			...
			let l = i._removeEmptyFolders(t), u = i._getFinalPath(l), d = await (e == "PUT" ? tn : j)(i.fetch, `${i.url}/object/${u}`, a, k({ headers: s }, o?.duplex ? { duplex: o.duplex } : {}));
			return {
				path: l,
				id: d.Id,
				fullPath: d.Key
	...
	async uploadToSignedUrl(e, t, n, r) {
		...
		return s.searchParams.set("token", t), i.handleOperation(async () => {
			let e, t = k(k({}, un), r), o = k(k({}, i.headers), { "x-upsert": String(t.upsert) }), c = t.metadata;
			...
			return {
				path: a,
				fullPath: (await tn(i.fetch, s.toString(), e, k({ headers: o }, t?.duplex ? { duplex: t.duplex } : {}))).Key
			};
		});
	}
	async createSignedUploadUrl(e, t) {
		...
		return n.handleOperation(async () => {
			let r = n._getFinalPath(e), i = k({}, n.headers);
			t?.upsert && (i["x-upsert"] = "true");
			let a = await j(n.fetch, `${n.url}/object/upload/sign/${r}`, {}, { headers: i }), o = new URL(n.url + a.url), s = o.searchParams.get("token");
			if (!s) throw new zt("No token returned by API");
			return {
	...
	async move(e, t, n) {
		var r = this;
		return r.handleOperation(async () => await j(r.fetch, `${r.url}/object/move`, {
			bucketId: r.bucketId,
			sourceKey: e,
			destinationKey: t,
	...
	async copy(e, t, n) {
		var r = this;
		return r.handleOperation(async () => ({ path: (await j(r.fetch, `${r.url}/object/copy`, {
			bucketId: r.bucketId,
			sourceKey: e,
			destinationKey: t,
			destinationBucket: n?.destinationBucket,
	...
	async createSignedUrl(e, t, n) {
		...
		return r.handleOperation(async () => {
			let i = r._getFinalPath(e), a = typeof n?.transform == "object" && n.transform !== null && Object.keys(n.transform).length > 0, o = await j(r.fetch, `${r.url}/object/sign/${i}`, k(k({ expiresIn: t }, a ? { transform: n.transform } : {}), n?.versionId == null ? {} : { versionId: n.versionId }), { headers: r.headers }), s = new URLSearchParams();
	...
	async createSignedUrls(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = await j(r.fetch, `${r.url}/object/sign/${r.bucketId}`, {
				expiresIn: t,
				paths: e
			...
			let o = a.toString();
			return i.map((e) => k(k({}, e), {}, { signedUrl: e.signedURL ? encodeURI(`${r.url}${e.signedURL}${o ? `&${o}` : ""}`) : null }));
		});
	}
	download(e, t, n) {
		...
		return new cn(() => A(this.fetch, `${this.url}/${r}/${o}${a ? `?${a}` : ""}`, {
			headers: this.headers,
			noResolveJson: !0
	...
	async info(e, t) {
		...
		let a = i.toString();
		return n.handleOperation(async () => qt(await A(n.fetch, `${n.url}/object/info/${r}${a ? `?${a}` : ""}`, { headers: n.headers })));
	}
	async exists(e) {
		var t = this;
		let n = t._getFinalPath(e);
		try {
			return await nn(t.fetch, `${t.url}/object/${n}`, { headers: t.headers }), {
				data: !0,
				error: null
			};
		} catch (e) {
			if (t.shouldThrowOnError) throw e;
			if (Bt(e)) {
				let t = e instanceof Vt ? e.status : e instanceof Ht ? e.originalError?.status : void 0;
	...
	async remove(e) {
		var t = this;
		return t.handleOperation(async () => await M(t.fetch, `${t.url}/object/${t.bucketId}`, { prefixes: e }, { headers: t.headers }));
	}
	async purgeCache(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = Yt(r._getFinalPath(e)), a = new URLSearchParams();
			t?.transformations && a.set("transformations", "true");
			let o = a.toString();
			return await M(r.fetch, `${r.url}/cdn/${i}${o ? `?${o}` : ""}`, {}, { headers: r.headers }, n);
		});
	}
	async list(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = t?.sortBy ? k(k({}, ln.sortBy), t.sortBy) : ln.sortBy, a = k(k(k({}, ln), t), {}, {
				sortBy: i,
				prefix: e || ""
			});
			return await j(r.fetch, `${r.url}/object/list/${r.bucketId}`, a, { headers: r.headers }, n);
		});
	}
	async listV2(e, t) {
		var n = this;
		return n.handleOperation(async () => {
			let r = k({}, e);
			return await j(n.fetch, `${n.url}/object/list-v2/${n.bucketId}`, r, { headers: n.headers }, t);
		});
	}
	encodeMetadata(e) {
		return JSON.stringify(e);
...
}, fn = { "X-Client-Info": "storage-js/2.116.0" }, pn = class extends rn {
	constructor(e, t = {}, n, r) {
		let i = new URL(e);
		...
		let a = i.href.replace(/\/$/, ""), o = k(k({}, fn), t);
		super(a, o, n, "storage");
	}
	async listBuckets(e) {
		var t = this;
		return t.handleOperation(async () => {
			...
			return await A(t.fetch, `${t.url}/bucket${n}`, { headers: t.headers });
		});
	}
	async getBucket(e) {
		var t = this;
		return t.handleOperation(async () => await A(t.fetch, `${t.url}/bucket/${e}`, { headers: t.headers }));
	}
	async createBucket(e, t = { public: !1 }) {
		var n = this;
		return n.handleOperation(async () => await j(n.fetch, `${n.url}/bucket`, {
			id: e,
			name: e,
			type: t.type,
			public: t.public,
	...
	async updateBucket(e, t) {
		var n = this;
		return n.handleOperation(async () => await tn(n.fetch, `${n.url}/bucket/${e}`, {
			id: e,
			name: e,
			public: t.public,
			file_size_limit: t.fileSizeLimit,
	...
	async emptyBucket(e) {
		var t = this;
		return t.handleOperation(async () => await j(t.fetch, `${t.url}/bucket/${e}/empty`, {}, { headers: t.headers }));
	}
	async deleteBucket(e) {
		var t = this;
		return t.handleOperation(async () => await M(t.fetch, `${t.url}/bucket/${e}`, {}, { headers: t.headers }));
	}
	async getBucketLifecycle(e) {
		var t = this;
		return t.handleOperation(async () => await A(t.fetch, t.bucketLifecycleUrl(e), { headers: t.headers }));
	}
	async updateBucketLifecycle(e, t) {
		var n = this;
		return n.handleOperation(async () => await tn(n.fetch, n.bucketLifecycleUrl(e), t, { headers: n.headers }));
	}
	async deleteBucketLifecycle(e) {
		var t = this;
		return t.handleOperation(async () => await M(t.fetch, t.bucketLifecycleUrl(e), {}, { headers: t.headers }));
	}
	async purgeBucketCache(e, t, n) {
		...
		return r.handleOperation(async () => {
			...
			let a = i.toString();
			return await M(r.fetch, `${r.url}/cdn/${Yt(e)}${a ? `?${a}` : ""}`, {}, { headers: r.headers }, n);
		});
	}
	bucketLifecycleUrl(e) {
		return `${this.url}/bucket/${Yt(e)}/lifecycle`;
	}
	...
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = k(k({}, fn), t);
		super(r, i, n, "storage");
	}
	async createBucket(e) {
		var t = this;
		return t.handleOperation(async () => await j(t.fetch, `${t.url}/bucket`, { name: e }, { headers: t.headers }));
	}
	async listBuckets(e) {
		var t = this;
		return t.handleOperation(async () => {
			...
			return await A(t.fetch, i, { headers: t.headers });
		});
	}
	async deleteBucket(e) {
		var t = this;
	...
	from(e) {
		var t = this;
		if (!Jt(e)) throw new zt("Invalid bucket name: File, folder, and bucket names must follow AWS object key naming guidelines and should avoid the use of any other characters.");
		let n = new Nt({
			baseUrl: this.url,
			catalogName: e,
			auth: {
	...
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = k(k({}, fn), {}, { "Content-Type": "application/json" }, t);
		super(r, i, n, "vectors");
	}
	async createIndex(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/CreateIndex`, e, { headers: t.headers }) || {});
	}
	async getIndex(e, t) {
		var n = this;
		return n.handleOperation(async () => await N.post(n.fetch, `${n.url}/GetIndex`, {
			vectorBucketName: e,
			indexName: t
		}, { headers: n.headers }));
	}
	async listIndexes(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/ListIndexes`, e, { headers: t.headers }));
	}
	async deleteIndex(e, t) {
		var n = this;
		return n.handleOperation(async () => await N.post(n.fetch, `${n.url}/DeleteIndex`, {
			vectorBucketName: e,
			indexName: t
		}, { headers: n.headers }) || {});
	}
	...
	async putVectors(e) {
		...
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/PutVectors`, e, { headers: t.headers }) || {});
	}
	async getVectors(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/GetVectors`, e, { headers: t.headers }));
	}
	async listVectors(e) {
		var t = this;
		...
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/ListVectors`, e, { headers: t.headers }));
	}
	async queryVectors(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/QueryVectors`, e, { headers: t.headers }));
	}
	async deleteVectors(e) {
		var t = this;
		if (e.keys.length < 1 || e.keys.length > 500) throw Error("Keys batch size must be between 1 and 500 items");
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/DeleteVectors`, e, { headers: t.headers }) || {});
	}
}, _n = class extends rn {
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = k(k({}, fn), {}, { "Content-Type": "application/json" }, t);
	...
	async createBucket(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/CreateVectorBucket`, { vectorBucketName: e }, { headers: t.headers }) || {});
	}
	async getBucket(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/GetVectorBucket`, { vectorBucketName: e }, { headers: t.headers }));
	}
	async listBuckets(e = {}) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/ListVectorBuckets`, e, { headers: t.headers }));
	}
	async deleteBucket(e) {
		var t = this;
		return t.handleOperation(async () => await N.post(t.fetch, `${t.url}/DeleteVectorBucket`, { vectorBucketName: e }, { headers: t.headers }) || {});
	}
}, vn = class extends _n {
	constructor(e, t = {}) {
	...
	async createIndex(e) {
		...
		return t().call(n, k(k({}, e), {}, { vectorBucketName: n.vectorBucketName }));
	}
	async listIndexes(e = {}) {
...
}, Sn = "2.116.0", P = 30 * 1e3, Cn = 3 * P, wn = 2 * P, Tn = "http://localhost:9999", En = "supabase.auth.token", Dn = { "X-Client-Info": `gotrue-js/${Sn}` }, On = "X-Supabase-Api-Version", kn = { "2024-01-01": {
	timestamp: Date.parse("2024-01-01T00:00:00.0Z"),
	name: "2024-01-01"
} }, An = /^([a-z0-9_-]{4})*($|[a-z0-9_-]{3}$|[a-z0-9_-]{2}$)$/i, jn = "sb_flow_id", Mn = class extends Error {
	constructor(e, t, n) {
...
function Gn(e) {
	return F(e) && e.name === "AuthRefreshDiscardedError";
}
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/helpers.js
function ar(e) {
...
var B = () => typeof window < "u" && typeof document < "u", V = {
	tested: !1,
	writable: !1
}, sr = () => {
	if (!B()) return !1;
	...
	try {
		globalThis.localStorage.setItem(e, e), globalThis.localStorage.removeItem(e), V.tested = !0, V.writable = !0;
	} catch {
...
var lr = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), ur = (e) => typeof e == "object" && !!e && "status" in e && "ok" in e && "json" in e && typeof e.json == "function", H = async (e, t, n) => {
...
function fr(e) {
	...
	for (let e = 0; e < t.length; e++) if (!An.test(t[e])) throw new qn("JWT not in base64url format");
	return {
...
var yr = /^[a-zA-Z0-9_-]{8,64}$/;
function br(e) {
	return typeof e == "string" && yr.test(e) ? e : null;
}
function xr() {
...
var Sr = (e, t) => `${e}-flow-${t}-code-verifier`, Cr = (e) => `${e}-flows-code-verifier`;
async function wr(e, t) {
	let n = await U(e, Cr(t));
	return Array.isArray(n) ? n.filter((e) => br(e) !== null) : [];
}
async function Tr(e, t, n, r, i) {
	await H(e, Sr(t, n), r);
	let a = (await wr(e, t)).filter((e) => e !== n);
	for (a.push(n); a.length > 5;) {
		let n = a.shift();
		await W(e, Sr(t, n)), i?.(n);
	}
	await H(e, Cr(t), a), await H(e, `${t}-code-verifier`, r);
}
async function Er(e, t, n) {
	if (n) {
		let r = await U(e, Sr(t, n));
		return {
			verifier: typeof r == "string" ? r : null,
			flowId: n
		};
	}
	let r = await U(e, `${t}-code-verifier`);
	return {
		verifier: typeof r == "string" ? r : null,
		flowId: null
	};
...
async function G(e, t, n) {
	...
	let i = Sr(t, n), a = await U(e, i);
	await W(e, i);
	let o = await wr(e, t), s = o.filter((e) => e !== n);
	s.length !== o.length && (s.length > 0 ? await H(e, Cr(t), s) : await W(e, Cr(t))), a != null && a === await U(e, r) && await W(e, r);
}
async function Dr(e, t) {
	let n = await wr(e, t);
	for (let r of n) await W(e, Sr(t, r));
	await W(e, Cr(t)), await W(e, `${t}-code-verifier`);
}
function Or(e, t) {
	...
	let o = r.includes("?") ? "&" : "?";
	return `${r}${o}${jn}=${encodeURIComponent(t)}${i}`;
}
async function kr(e, t, n = !1, r) {
	let i = gr(), a = i;
	n && (a += "/recovery");
	let o = xr();
	await Tr(e, t, o, a, r);
...
var Ar = /^2[0-9]{3}-(0[1-9]|1[0-2])-(0[1-9]|1[0-9]|2[0-9]|3[0-1])$/i;
function jr(e) {
	let t = e.headers.get(On);
	if (!t || !t.match(Ar)) return null;
	try {
...
var Pr = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function K(e) {
	if (!Pr.test(e)) throw Error("@supabase/auth-js: Expected parameter to be UUID but is not");
}
function q(e) {
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/fetch.js
var J = (e) => {
	if (typeof e == "object" && e) {
		let t = e;
		if (typeof t.msg == "string") return t.msg;
...
async function Br(e) {
	if (!ur(e)) throw new Hn(J(e), 0);
	let t;
	try {
		t = await e.json();
	} catch (t) {
		throw zr.includes(e.status) ? new Hn(e.statusText || `HTTP ${e.status}`, e.status) : new I(J(t), t);
	}
	if (zr.includes(e.status)) throw new Hn(J(t), e.status);
	let n, r = jr(e);
	if (r && r.getTime() >= kn["2024-01-01"].timestamp && typeof t == "object" && t && typeof t.code == "string" ? n = t.code : typeof t == "object" && t && typeof t.error_code == "string" && (n = t.error_code), !n) {
		if (typeof t == "object" && t && typeof t.weak_password == "object" && t.weak_password && Array.isArray(t.weak_password.reasons) && t.weak_password.reasons.length && t.weak_password.reasons.reduce((e, t) => e && typeof t == "string", !0)) throw new Kn(J(t), e.status, t.weak_password.reasons);
	} else if (n === "weak_password") throw new Kn(J(t), e.status, t.weak_password?.reasons || []);
	else if (n === "session_not_found") throw new R();
	throw new Nn(J(t), e.status || 500, n);
}
var Vr = (e, t, n, r) => {
	let i = {
		method: e,
...
async function Y(e, t, n, r) {
	...
	i["X-Supabase-Api-Version"] || (i[On] = kn["2024-01-01"].name), r?.jwt && (i.Authorization = `Bearer ${r.jwt}`);
	let a = r?.query ?? {};
	r?.redirectTo && (a.redirect_to = r.redirectTo);
	let o = await Hr(e, t, n + (Object.keys(a).length ? "?" + new URLSearchParams(a).toString() : ""), {
		headers: i,
...
async function Hr(e, t, n, r, i, a) {
	...
	if (s.ok || await Br(s), r?.noResolveJson) return s;
	try {
		return await s.json();
	} catch (e) {
		await Br(e);
...
function X(e) {
	let t = null;
	qr(e) && (t = Object.assign({}, e), e.expires_at || (t.expires_at = ar(e.expires_in)));
...
function Gr(e) {
	let { action_link: t, email_otp: r, hashed_token: i, redirect_to: a, verification_type: o } = e, s = n(e, [
		"action_link",
		"email_otp",
		"hashed_token",
		"redirect_to",
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/types.js
var Jr = [
	"global",
	"local",
	"others"
	...
	constructor({ url: e = "", headers: t = {}, fetch: n, experimental: r }) {
		this.url = e, this.headers = t, this.fetch = lr(n), this.experimental = r ?? {}, this.mfa = {
	...
	async signOut(e, t = Jr[0]) {
		if (Jr.indexOf(t) < 0) throw Error(`@supabase/auth-js: Parameter scope must be one of ${Jr.join(", ")}`);
		try {
	...
	async generateLink(e) {
		try {
			let { options: t } = e, r = n(e, ["options"]), i = Object.assign(Object.assign({}, r), t);
			return "newEmail" in r && (i.new_email = r?.newEmail, delete i.newEmail), await Y(this.fetch, "POST", `${this.url}/admin/generate_link`, {
				body: i,
				headers: this.headers,
				xform: Gr,
				redirectTo: t?.redirectTo
	...
	async listUsers(e) {
		try {
			let t = {
				...
				lastPage: 0,
				total: 0
			}, n = await Y(this.fetch, "GET", `${this.url}/admin/users`, {
				headers: this.headers,
				noResolveJson: !0,
				query: {
	...
	async _listFactors(e) {
		K(e.userId);
		try {
			let { data: t, error: n } = await Y(this.fetch, "GET", `${this.url}/admin/users/${e.userId}/factors`, {
				headers: this.headers,
				xform: (e) => ({
					data: { factors: e },
					error: null
	...
	async _deleteFactor(e) {
		...
		try {
			return {
				data: await Y(this.fetch, "DELETE", `${this.url}/admin/users/${e.userId}/factors/${e.id}`, { headers: this.headers }),
				error: null
			};
		} catch (e) {
			if (F(e)) return {
	...
	async _listOAuthClients(e) {
		try {
			let t = {
				...
				lastPage: 0,
				total: 0
			}, n = await Y(this.fetch, "GET", `${this.url}/admin/oauth/clients`, {
				headers: this.headers,
				noResolveJson: !0,
				query: {
					page: (e?.page)?.toString() ?? "",
	...
	async _getOAuthClient(e) {
		try {
			return await Y(this.fetch, "GET", `${this.url}/admin/oauth/clients/${e}`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _updateOAuthClient(e, t) {
		try {
			return await Y(this.fetch, "PUT", `${this.url}/admin/oauth/clients/${e}`, {
				body: t,
				headers: this.headers,
				xform: (e) => ({
					data: e,
	...
	async _deleteOAuthClient(e) {
		try {
			return await Y(this.fetch, "DELETE", `${this.url}/admin/oauth/clients/${e}`, {
				headers: this.headers,
				noResolveJson: !0
			}), {
				data: null,
	...
	async _regenerateOAuthClientSecret(e) {
		try {
			return await Y(this.fetch, "POST", `${this.url}/admin/oauth/clients/${e}/regenerate_secret`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _listCustomProviders(e) {
		try {
			let t = {};
			return e?.type && (t.type = e.type), await Y(this.fetch, "GET", `${this.url}/admin/custom-providers`, {
				headers: this.headers,
				query: t,
				xform: (e) => ({
	...
	async _getCustomProvider(e) {
		try {
			return await Y(this.fetch, "GET", `${this.url}/admin/custom-providers/${e}`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _updateCustomProvider(e, t) {
		try {
			return await Y(this.fetch, "PUT", `${this.url}/admin/custom-providers/${e}`, {
				body: t,
				headers: this.headers,
				xform: (e) => ({
					data: e,
	...
	async _deleteCustomProvider(e) {
		try {
			return await Y(this.fetch, "DELETE", `${this.url}/admin/custom-providers/${e}`, {
				headers: this.headers,
				noResolveJson: !0
			}), {
				data: null,
	...
	async _adminListPasskeys(e) {
		q(this.experimental), K(e.userId);
		try {
			return await Y(this.fetch, "GET", `${this.url}/admin/users/${e.userId}/passkeys`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _adminDeletePasskey(e) {
		q(this.experimental), K(e.userId), K(e.passkeyId);
		try {
			return await Y(this.fetch, "DELETE", `${this.url}/admin/users/${e.userId}/passkeys/${e.passkeyId}`, {
				headers: this.headers,
				noResolveJson: !0
			}), {
				data: null,
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/local-storage.js
function Xr(e = {}) {
	return {
		getItem: (t) => e[t] || null,
		setItem: (t, n) => {
...
globalThis && sr() && globalThis.localStorage && globalThis.localStorage.getItem("supabase.gotrue-js.locks.debug");
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/polyfills.js
function Qr() {
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/web3/ethereum.js
function $r(e) {
...
function ni(e) {
	...
	let f = $r(e.address), p = `${l ? `${l}://${n}` : n} wants you to sign in with your Ethereum account:\n${f}\n\n${e.statement ? `${e.statement}\n` : ""}`, m = `URI: ${u}\nVersion: ${d}\nChain ID: ${t}${a ? `\nNonce: ${a}` : ""}\nIssued At: ${i.toISOString()}`;
...
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/lib/webauthn.errors.js
...
function ii({ error: e, options: t }) {
	...
	if (e.name === "AbortError") {
		if (t.signal instanceof AbortSignal) return new Q({
			message: "Registration ceremony was sent an abort signal",
			code: "ERROR_CEREMONY_ABORTED",
			cause: e
		});
	} else if (e.name === "ConstraintError") {
		if (n.authenticatorSelection?.requireResidentKey === !0) return new Q({
			message: "Discoverable credentials were required but no available authenticator supported it",
			code: "ERROR_AUTHENTICATOR_MISSING_DISCOVERABLE_CREDENTIAL_SUPPORT",
			cause: e
		});
		if (t.mediation === "conditional" && n.authenticatorSelection?.userVerification === "required") return new Q({
			message: "User verification was required during automatic registration but it could not be performed",
			code: "ERROR_AUTO_REGISTER_USER_VERIFICATION_FAILURE",
			cause: e
		});
		if (n.authenticatorSelection?.userVerification === "required") return new Q({
			message: "User verification was required but no available authenticator supported it",
			code: "ERROR_AUTHENTICATOR_MISSING_USER_VERIFICATION_SUPPORT",
			cause: e
		});
	} else if (e.name === "InvalidStateError") return new Q({
		message: "The authenticator was previously registered",
		code: "ERROR_AUTHENTICATOR_PREVIOUSLY_REGISTERED",
		cause: e
	});
	else if (e.name === "NotAllowedError") return new Q({
		message: e.message,
		code: "ERROR_PASSTHROUGH_SEE_CAUSE_PROPERTY",
		cause: e
	});
	else if (e.name === "NotSupportedError") return n.pubKeyCredParams.filter((e) => e.type === "public-key").length === 0 ? new Q({
		...
		if (!di(t)) return new Q({
			message: `${window.location.hostname} is an invalid domain`,
			code: "ERROR_INVALID_DOMAIN",
			cause: e
		});
		if (n.rp.id !== t) return new Q({
			message: `The RP ID "${n.rp.id}" is invalid for this domain`,
			code: "ERROR_INVALID_RP_ID",
			cause: e
		});
	} else if (e.name === "TypeError") {
		if (n.user.id.byteLength < 1 || n.user.id.byteLength > 64) return new Q({
			message: "User ID was not between 1 and 64 characters",
			code: "ERROR_INVALID_USER_ID_LENGTH",
			cause: e
		});
	} else if (e.name === "UnknownError") return new Q({
...
function si(e) {
	...
	let { challenge: t, user: r, excludeCredentials: i } = e, a = n(e, [
		"challenge",
		"user",
		"excludeCredentials"
	]), o = rr(t).buffer, s = Object.assign(Object.assign({}, r), { id: rr(r.id).buffer }), c = Object.assign(Object.assign({}, a), {
		challenge: o,
		user: s
	});
	if (i && i.length > 0) {
...
function ci(e) {
	...
	let { challenge: t, allowCredentials: r } = e, i = n(e, ["challenge", "allowCredentials"]), a = rr(t).buffer, o = Object.assign(Object.assign({}, i), { challenge: a });
	if (r && r.length > 0) {
...
function li(e) {
	...
	return {
		...
		rawId: e.id,
		response: {
			attestationObject: z(new Uint8Array(e.response.attestationObject)),
			clientDataJSON: z(new Uint8Array(e.response.clientDataJSON))
		},
		type: "public-key",
...
function ui(e) {
	...
	return {
		...
		rawId: e.id,
		response: {
			authenticatorData: z(new Uint8Array(r.authenticatorData)),
			clientDataJSON: z(new Uint8Array(r.clientDataJSON)),
			signature: z(new Uint8Array(r.signature)),
			userHandle: r.userHandle ? z(new Uint8Array(r.userHandle)) : void 0
		},
		type: "public-key",
...
async function pi(e) {
	try {
		...
		return t ? t instanceof PublicKeyCredential ? {
			...
			data: null,
			error: new ri("Empty credential response", t)
		};
	} catch (t) {
		return {
			data: null,
...
var bi = class {
	...
	async _challenge({ factorId: e, webauthn: t, friendlyName: n, signal: r }, i) {
		try {
			...
			let s = r ?? oi.createNewAbortSignal();
			...
			switch (a.webauthn.type) {
				case "create": {
					let { data: t, error: n } = await pi({
						publicKey: vi(a.webauthn.credential_options.publicKey, i?.create),
						signal: s
					});
					return t ? {
						data: {
				...
				case "request": {
					let t = yi(a.webauthn.credential_options.publicKey, i?.request), { data: n, error: r } = await mi(Object.assign(Object.assign({}, a.webauthn.credential_options), {
						publicKey: t,
						signal: s
					}));
					return n ? {
			...
			return F(e) ? {
				...
				data: null,
				error: new I("Unexpected error in challenge", e)
			};
		}
	}
	...
	async _authenticate({ factorId: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		...
		try {
			if (!fi()) return {
				data: null,
				error: new I("Browser does not support WebAuthn", null)
			};
			...
			return F(e) ? {
				...
				data: null,
				error: new I("Unexpected error in authenticate", e)
			};
		}
	}
	async _register({ friendlyName: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		...
		try {
			...
			return F(e) ? {
				...
				data: null,
				error: new I("Unexpected error in register", e)
			};
		}
	}
};
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/GoTrueClient.js
Qr();
var xi = {
	url: Tn,
	storageKey: En,
	...
	get jwks() {
		return Si[this.storageKey]?.jwks ?? { keys: [] };
	}
	set jwks(e) {
		Si[this.storageKey] = Object.assign(Object.assign({}, Si[this.storageKey]), { jwks: e });
	}
	get jwks_cached_at() {
		return Si[this.storageKey]?.cachedAt ?? -(2 ** 53 - 1);
	}
	set jwks_cached_at(e) {
		Si[this.storageKey] = Object.assign(Object.assign({}, Si[this.storageKey]), { cachedAt: e });
	}
	constructor(t) {
		var n;
		...
		let r = Object.assign(Object.assign({}, xi), t);
		if (this.storageKey = r.storageKey, this.instanceID = e.nextInstanceID[this.storageKey] ?? 0, e.nextInstanceID[this.storageKey] = this.instanceID + 1, this.logDebugMessages = !!r.debug, typeof r.debug == "function" && (this.logger = r.debug), this.instanceID > 0 && B()) {
		...
		}, this.persistSession ? (r.storage ? this.storage = r.storage : sr() ? this.storage = globalThis.localStorage : (this.memoryStorage = {}, this.storage = Xr(this.memoryStorage)), r.userStorage && (this.userStorage = r.userStorage)) : (this.memoryStorage = {}, this.storage = Xr(this.memoryStorage)), B() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
			try {
	...
	async _initialize() {
		try {
			let e = {}, t = "none";
			if (B() && (e = cr(window.location.href), this._isImplicitGrantCallback(e) ? t = "implicit" : await this._isPKCECallback(e) && (t = "pkce")), B() && this.detectSessionInUrl && t !== "none") {
			...
			return F(e) ? this._returnResult({ error: e }) : this._returnResult({ error: new I("Unexpected error during initialization", e) });
		} finally {
	...
	async signInAnonymously(e) {
		try {
			let { data: t, error: n } = await Y(this.fetch, "POST", `${this.url}/signup`, {
				headers: this.headers,
				body: {
					data: e?.options?.data ?? {},
	...
	async signUp(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				this.flowType === "pkce" && ([o, s, t] = await this._getCodeChallengeAndMethod()), n = await Y(this.fetch, "POST", `${this.url}/signup`, {
					headers: this.headers,
			...
			if (i || !r) return await G(this.storage, this.storageKey, t), this._returnResult({
				data: {
					user: null,
					session: null
				},
			...
			if (await G(this.storage, this.storageKey, t), F(e)) return this._returnResult({
				data: {
					user: null,
					session: null
				},
	...
	async signInWithEthereum(e) {
		...
		if ("message" in e) t = e.message, n = e.signature;
			...
			let u = $r(l[0]), d = o?.signInWithEthereum?.chainId;
			d ||= ei(await s.request({ method: "eth_chainId" })), t = ni({
				domain: c.host,
				address: u,
				statement: a,
				uri: c.href,
		...
		try {
			let { data: r, error: i } = await Y(this.fetch, "POST", `${this.url}/token?grant_type=web3`, {
				headers: this.headers,
				body: Object.assign({
					chain: "ethereum",
					message: t,
	...
	async _exchangeCodeForSession(e, t) {
		let n = t?.flowId != null, r = n ? br(t?.flowId) : B() ? br(cr(window.location.href)[jn]) : null;
		n && !r && this._debug("#_exchangeCodeForSession()", "provided flowId is not a valid flow id", t?.flowId);
		let { verifier: i, flowId: a } = n && !r ? {
			verifier: null,
			flowId: null
		} : await Er(this.storage, this.storageKey, r), [o, s] = (i ?? "").split("/");
		try {
			if (!o && this.flowType === "pkce") throw new Vn();
			let { data: t, error: n } = await Y(this.fetch, "POST", `${this.url}/token?grant_type=pkce`, {
				headers: this.headers,
				body: {
					auth_code: e,
					code_verifier: o
			...
			if (await G(this.storage, this.storageKey, a), n) throw n;
			...
			if (await G(this.storage, this.storageKey, a), F(e)) return this._returnResult({
				data: {
					user: null,
					session: null,
					redirectType: null
	...
	async signInWithIdToken(e) {
		try {
			let { options: t, provider: n, token: r, access_token: i, nonce: a } = e, { data: o, error: s } = await Y(this.fetch, "POST", `${this.url}/token?grant_type=id_token`, {
				headers: this.headers,
				body: {
					provider: n,
					id_token: r,
	...
	async signInWithOtp(e) {
		...
		try {
			if ("email" in e) {
				...
				let { error: o } = await Y(this.fetch, "POST", `${this.url}/otp`, {
					headers: this.headers,
					body: {
						email: n,
						data: r?.data ?? {},
			...
			if ("phone" in e) {
				let { phone: t, options: n } = e, { data: r, error: i } = await Y(this.fetch, "POST", `${this.url}/otp`, {
					headers: this.headers,
					body: {
						phone: t,
						data: n?.data ?? {},
	...
	async verifyOtp(e) {
		try {
			...
			let { data: r, error: i } = await Y(this.fetch, "POST", `${this.url}/verify`, {
				headers: this.headers,
	...
	async signInWithSSO(e) {
		...
		try {
			...
			let i = await Y(this.fetch, "POST", `${this.url}/sso`, {
			...
			return i.data?.url && B() && !e.options?.skipBrowserRedirect && window.location.assign(i.data.url), this._returnResult(i);
		} catch (e) {
			if (await G(this.storage, this.storageKey, t), F(e)) return this._returnResult({
				data: null,
				error: e
	...
	async _reauthenticate() {
		try {
			return await this._useSession(async (e) => {
				...
				if (n) throw n;
				if (!t) throw new R();
				let { error: r } = await Y(this.fetch, "GET", `${this.url}/reauthenticate`, {
					headers: this.headers,
					jwt: t.access_token
				});
				return this._returnResult({
	...
	async resend(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				let { error: c } = await Y(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						email: r,
						type: i,
				...
				return c && await G(this.storage, this.storageKey, t), this._returnResult({
					data: {
						user: null,
						session: null
					},
				...
				let { phone: t, type: r, options: i } = e, { data: a, error: o } = await Y(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						phone: t,
						type: r,
	...
	async __loadSession() {
		...
		try {
			let e = null, t = await U(this.storage, this.storageKey);
			...
			let n = e.expires_at ? e.expires_at * 1e3 - Date.now() < Cn : !1;
			if (this._debug("#__loadSession()", `session has${n ? "" : " not"} expired`, "expires_at", e.expires_at), !n) {
				if (this.userStorage) {
					let t = await U(this.userStorage, this.storageKey + "-user");
					t?.user ? e.user = t.user : e.user = Ir();
				}
				if (this.storage.isServer && e.user && !e.user.__isUserNotAvailableProxy) {
					let t = { value: this.suppressGetSessionWarning };
					e.user = Lr(e.user, t), t.value && (this.suppressGetSessionWarning = !0);
				}
				return {
					data: { session: e },
					error: null
			...
			if (i) {
				if (e.expires_at && e.expires_at * 1e3 > Date.now()) {
					let t = await U(this.storage, this.storageKey);
	...
	async _getUser(e) {
		try {
			...
			if (F(e)) return Fn(e) && await this._removeSession(), this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _updateUser(e, t = {}) {
		...
		try {
			...
			if (await G(this.storage, this.storageKey, n), F(e)) return this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _setSession(e) {
		try {
			if (!e.access_token || !e.refresh_token) throw new R();
	...
	async _refreshSession(e) {
		try {
			return await this._useSession(async (t) => {
				...
				if (!e?.refresh_token) throw new R();
	...
	async _getSessionFromURL(e, t) {
		try {
			if (!B()) throw new Rn("No browser detected.");
			...
			if (t === "pkce") {
				...
				let { data: t, error: n } = await this._exchangeCodeForSession(e.code, { flowId: e[jn] });
				if (n) throw n;
				let r = new URL(window.location.href);
				return r.searchParams.delete("code"), r.searchParams.delete(jn), window.history.replaceState(window.history.state, "", r.toString()), {
					data: {
						session: t.session,
	...
	async _isPKCECallback(e) {
		if (!e.code) return !1;
		let t = br(e[jn]);
		return t && await U(this.storage, Sr(this.storageKey, t)) ? !0 : !!await U(this.storage, `${this.storageKey}-code-verifier`);
	}
	...
	async _signOut({ scope: e } = { scope: "global" }) {
		return await this._useSession(async (t) => {
			...
			if (a) {
				...
				if (t && !(Pn(t) && (t.status === 404 || t.status === 401 || t.status === 403) || Fn(t))) return e !== "others" && await n(), this._returnResult({ error: t });
			}
	...
	async _emitInitialSession(e) {
		return await this._useSession(async (t) => {
			try {
				...
				if (await this.stateChangeEmitters.get(e)?.callback("INITIAL_SESSION", null), this._debug("INITIAL_SESSION", "callback id", e, "error", t), Gn(t)) return;
				Fn(t) || Un(t) || Pn(t) && (t.code === "refresh_token_not_found" || t.code === "refresh_token_already_used" || t.code === "session_expired") ? console.warn(t) : console.error(t);
			}
		});
	}
	async resetPasswordForEmail(e, t = {}) {
		...
		try {
			...
			if (await G(this.storage, this.storageKey, i), F(e)) return this._returnResult({
				data: null,
				error: e
			});
			throw e;
	...
	async linkIdentityOAuth(e) {
		...
		try {
			let { data: n, error: r } = await this._useSession(async (n) => {
				...
				return t = o, await Y(this.fetch, "GET", a, {
					headers: this.headers,
					jwt: r.session?.access_token ?? void 0
				});
			});
			if (r) throw r;
			return B() && !e.options?.skipBrowserRedirect && window.location.assign(n?.url), this._returnResult({
				data: {
					provider: e.provider,
					url: n?.url,
					flowId: t
			...
			if (F(n)) return this._returnResult({
				data: {
					provider: e.provider,
					url: null,
					flowId: t
	...
	async linkIdentityIdToken(e) {
		return await this._useSession(async (t) => {
			try {
				...
				if (n) throw n;
				let { options: i, provider: a, token: o, access_token: s, nonce: c } = e, { data: l, error: u } = await Y(this.fetch, "POST", `${this.url}/token?grant_type=id_token`, {
					headers: this.headers,
				...
				if (await G(this.storage, this.storageKey, null), F(e)) return this._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	async _refreshAccessToken(e) {
		...
		try {
			let n = Date.now();
			return await mr(async (n) => (n > 0 && await pr(200 * 2 ** (n - 1)), this._debug(t, "refreshing attempt", n), await Y(this.fetch, "POST", `${this.url}/token?grant_type=refresh_token`, {
			...
			if (this._debug(t, "error", e), F(e)) return this._returnResult({
				data: {
					session: null,
					user: null
				},
	...
	async _handleProviderSignIn(e, t) {
		...
		return this._debug("#_handleProviderSignIn()", "provider", e, "options", t, "url", n), B() && !t.skipBrowserRedirect && window.location.assign(n), {
			data: {
				provider: e,
				url: n,
				flowId: r
	...
	async _recoverAndRefresh() {
		...
		try {
			...
			if (t && this.userStorage) {
				let e = await U(this.userStorage, this.storageKey + "-user");
				!this.storage.isServer && Object.is(this.storage, this.userStorage) && !e && (e = { user: t.user }, await H(this.userStorage, this.storageKey + "-user", e)), t.user = e?.user ?? Ir();
			} else if (t && !t.user && !t.user) {
				let e = await U(this.storage, this.storageKey + "-user");
				e && e?.user ? (t.user = e.user, await W(this.storage, this.storageKey + "-user"), await H(this.storage, this.storageKey, t)) : t.user = Ir();
			}
			...
			let n = (t.expires_at ?? Infinity) * 1e3 - Date.now() < Cn;
			if (this._debug(e, `session has${n ? "" : " not"} expired with margin of ${Cn}s`), n) {
			...
			this._debug(e, "error", t), Un(t) ? console.warn(t) : console.error(t);
			return;
		} finally {
			this._debug(e, "end");
		}
	...
	async _callRefreshToken(e) {
		...
		this._debug(r, "begin");
		try {
			this.refreshingDeferred = new dr(), this.refreshingDeferred.promise.then(void 0, () => {});
			let t = await U(this.storage, this.storageKey), { data: n, error: i } = await this._refreshAccessToken(e);
			if (i) throw i;
			if (!n.session) throw new R();
			let a = await U(this.storage, this.storageKey);
			...
			if (await this._saveSession(n.session), this._sessionRemovalEpoch !== o) {
				this._debug(r, "commit guard (post-save): _removeSession ran during _saveSession, undoing write"), await W(this.storage, this.storageKey), this.userStorage && await W(this.userStorage, this.storageKey + "-user");
				let e = {
					data: null,
					error: new Wn()
				};
			...
			if (this._debug(r, "error", i), F(i)) {
				let n = {
					data: null,
					error: i
				};
				if (!Un(i)) {
					let e = await U(this.storage, this.storageKey);
	...
	async _saveSession(e) {
		...
		if (this.userStorage) {
			!n && t.user && await H(this.userStorage, this.storageKey + "-user", { user: t.user });
			let e = Object.assign({}, t);
			delete e.user;
			let r = Rr(e);
			await H(this.storage, this.storageKey, r);
		} else {
			let e = Rr(t);
			await H(this.storage, this.storageKey, e);
		}
	}
	async _removeSession() {
		this._sessionRemovalEpoch += 1, this._debug("#_removeSession()"), this.lastRefreshFailure = null, this.suppressGetSessionWarning = !1, await W(this.storage, this.storageKey), await Dr(this.storage, this.storageKey), await W(this.storage, this.storageKey + "-user"), this.userStorage && await W(this.userStorage, this.storageKey + "-user"), await this._notifyAllSubscribers("SIGNED_OUT", null);
	}
	...
	async _startAutoRefresh() {
		...
		let e = setInterval(() => this._autoRefreshTokenTick(), P);
					...
					try {
						...
						try {
							return await this._useSession(async (t) => {
								...
								let r = Math.floor((n.expires_at * 1e3 - e) / P);
								this._debug("#_autoRefreshTokenTick()", `access token expires in ${r} ticks, a tick lasts ${P}ms, refresh threshold is 3 ticks`), r <= 3 && await this._callRefreshToken(n.refresh_token);
							});
						} catch (e) {
	...
	async _handleVisibilityChange() {
		if (this._debug("#_handleVisibilityChange()"), !B() || !(window != null && window.addEventListener)) return this.autoRefreshToken && this.startAutoRefresh(), !1;
		try {
	...
	_maybeAppendFlowIdToRedirect(e, t) {
		return !e || !t || !this.experimental.appendPkceFlowIdToRedirects ? e ?? void 0 : Or(e, t);
	}
	async _getCodeChallengeAndMethod(e = !1) {
		return kr(this.storage, this.storageKey, e, (e) => this._debug("#_getCodeChallengeAndMethod()", "evicted oldest pending PKCE verifier slot", e));
	}
	async _unenroll(e) {
		try {
			return await this._useSession(async (t) => {
	...
	async _enroll(e) {
		try {
			return await this._useSession(async (t) => {
				...
				}, e.factorType === "phone" ? { phone: e.phone } : e.factorType === "totp" ? { issuer: e.issuer } : {}), { data: a, error: o } = await Y(this.fetch, "POST", `${this.url}/factors`, {
					body: i,
					headers: this.headers,
					jwt: n?.session?.access_token
				});
		...
		let t = async () => {
			try {
				return await this._useSession(async (t) => {
					...
					let i = await Y(this.fetch, "POST", `${this.url}/factors/${e.factorId}/challenge`, {
						body: e,
						headers: this.headers,
						jwt: n?.session?.access_token
					});
					...
					switch (a.webauthn.type) {
						...
						case "request": return {
							data: Object.assign(Object.assign({}, a), { webauthn: Object.assign(Object.assign({}, a.webauthn), { credential_options: Object.assign(Object.assign({}, a.webauthn.credential_options), { publicKey: ci(a.webauthn.credential_options.publicKey) }) }) }),
							error: null
						};
					}
				});
	...
	async _getRecoveryCodesStatus() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await Y(this.fetch, "GET", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
	...
	async _generateRecoveryCodes(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await Y(this.fetch, "POST", `${this.url}/factors/recovery-codes`, {
		...
		let t = async () => {
			try {
				return await this._useSession(async (t) => {
					...
					let { data: i, error: a } = await Y(this.fetch, "POST", `${this.url}/factors/recovery-codes/verify`, {
						body: { code: e.code },
						headers: this.headers,
						jwt: n?.session?.access_token
					});
					...
					let o = Object.assign({ expires_at: ar(i.expires_in) }, i);
	...
	async _regenerateRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await Y(this.fetch, "POST", `${this.url}/factors/recovery-codes/regenerate`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
	...
	async _unenrollRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await Y(this.fetch, "DELETE", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
		...
		try {
			return await this._useSession(async (t) => {
				...
				return r ? this._returnResult({
					data: null,
					error: r
				}) : n ? await Y(this.fetch, "GET", `${this.url}/oauth/authorizations/${e}`, {
					headers: this.headers,
					jwt: n.access_token,
					xform: (e) => ({
						data: e,
	...
	async _approveAuthorization(e, t) {
		try {
			return await this._useSession(async (n) => {
				...
				let a = await Y(this.fetch, "POST", `${this.url}/oauth/authorizations/${e}/consent`, {
					headers: this.headers,
					jwt: r.access_token,
					body: { action: "approve" },
					xform: (e) => ({
				...
				return a.data && a.data.redirect_url && B() && !t?.skipBrowserRedirect && window.location.assign(a.data.redirect_url), a;
			});
		} catch (e) {
			if (F(e)) return this._returnResult({
				data: null,
		...
		try {
			return await this._useSession(async (e) => {
				...
				return n ? this._returnResult({
					data: null,
					error: n
				}) : t ? await Y(this.fetch, "GET", `${this.url}/user/oauth/grants`, {
					headers: this.headers,
					jwt: t.access_token,
					xform: (e) => ({
						data: e,
		...
		try {
			return await this._useSession(async (t) => {
				...
				return r ? this._returnResult({
					data: null,
					error: r
				}) : n ? (await Y(this.fetch, "DELETE", `${this.url}/user/oauth/grants`, {
					headers: this.headers,
					jwt: n.access_token,
	...
	async fetchJwk(e, t = { keys: [] }) {
		...
		let { data: i, error: a } = await Y(this.fetch, "GET", `${this.url}/.well-known/jwks.json`, { headers: this.headers });
		if (a) throw a;
	...
	async getClaims(e, t = {}) {
		try {
			...
			let l = Nr(r.alg), u = await crypto.subtle.importKey("jwk", c, l, !0, ["verify"]);
	...
	async signInWithPasskey(e) {
		q(this.experimental);
		try {
			if (!fi()) return this._returnResult({
				data: null,
				error: new I("Browser does not support WebAuthn", null)
			});
			...
			let { data: r, error: i } = await mi({
				publicKey: ci(t.options),
				signal: e?.options?.signal ?? oi.createNewAbortSignal()
			});
			if (i || !r) return this._returnResult({
				data: null,
				error: i ?? new I("WebAuthn ceremony failed", null)
			});
			let a = ui(r);
	...
	async _startPasskeyRegistration() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await Y(this.fetch, "POST", `${this.url}/passkeys/registration/options`, {
					headers: this.headers,
					jwt: t.access_token,
					body: {}
				});
	...
	async _verifyPasskeyRegistration(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await Y(this.fetch, "POST", `${this.url}/passkeys/registration/verify`, {
					headers: this.headers,
					jwt: n.access_token,
					body: {
	...
	async _startPasskeyAuthentication(e) {
		q(this.experimental);
		try {
			let { data: t, error: n } = await Y(this.fetch, "POST", `${this.url}/passkeys/authentication/options`, {
				headers: this.headers,
	...
	async _verifyPasskeyAuthentication(e) {
		q(this.experimental);
		try {
			let { data: t, error: n } = await Y(this.fetch, "POST", `${this.url}/passkeys/authentication/verify`, {
				headers: this.headers,
				body: {
					challenge_id: e.challengeId,
					credential: e.credential
	...
	async _listPasskeys() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await Y(this.fetch, "GET", `${this.url}/passkeys`, {
					headers: this.headers,
					jwt: t.access_token,
					xform: (e) => ({
						data: e,
	...
	async _updatePasskey(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await Y(this.fetch, "PATCH", `${this.url}/passkeys/${e.passkeyId}`, {
					headers: this.headers,
					jwt: n.access_token,
	...
	async _deletePasskey(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { error: i } = await Y(this.fetch, "DELETE", `${this.url}/passkeys/${e.passkeyId}`, {
					headers: this.headers,
					jwt: n.access_token,
					noResolveJson: !0
				});
...
wi.nextInstanceID = {};
//#endregion
//#region ../validation/node_modules/@supabase/auth-js/dist/module/AuthClient.js
var Ti = wi, Ei = "2.116.0", Di = "", Oi;
if (typeof Deno < "u") Di = "deno", Oi = Deno.version?.deno;
else if (typeof document < "u") Di = "web";
else if (typeof navigator < "u" && navigator.product === "ReactNative") Di = "react-native";
else {
	var ki;
	Di = "node";
	let e = globalThis.process;
	Oi = e == null || (ki = e.version) == null ? void 0 : ki.replace(/^v/, "");
}
var Ai = [`runtime=${Di}`];
Oi && Ai.push(`runtime-version=${Oi}`);
var ji = { headers: { "X-Client-Info": `supabase-js/${Ei}; ${Ai.join("; ")}` } }, Mi = { schema: "public" }, Ni = {
	autoRefreshToken: !0,
	persistSession: !0,
	detectSessionInUrl: !0,
	flowType: "implicit"
...
var Gi = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), Ki = () => Headers, qi = (e) => e.startsWith("sb_publishable_") || e.startsWith("sb_secret_"), Ji = "sb_temp_", Yi = /* @__PURE__ */ new Set(), Xi = (e) => {
	if (!e.startsWith("sb_") || qi(e) || e.startsWith(Ji)) return;
	let t = e.match(/^sb_[a-zA-Z0-9]+_/)?.[0] ?? "unknown";
	Yi.has(t) || (Yi.add(t), console.warn("@supabase/supabase-js: Unrecognized Supabase API key format. The client will proceed and send this key as-is; if you see authentication errors you may need to upgrade @supabase/supabase-js to a version that recognizes this key type."));
}, Zi = (e, t, n, r, i, a) => {
	let o = Gi(r), s = Ki(), c = i?.enabled === !0, l = i?.respectSamplingDecision !== !1, u = c ? zi(t) : null, d = !(a?.omitApiKeyAsBearer && qi(e));
	return async (t, r) => {
...
var ra = !1;
function ia(e) {
	ra || typeof e != "object" || !e || !("schema" in e) || e.schema === void 0 || (ra = !0, console.warn("@supabase/supabase-js: The \"schema\" option must be nested under \"db\", e.g. createClient(url, key, { db: { schema: 'myschema' } }). A top-level \"schema\" is ignored and queries go to the default schema."));
}
function aa(e, t) {
	let { db: n, auth: r, realtime: i, global: a } = e, { db: o, auth: s, realtime: c, global: l } = t, u = ta(e.tracePropagation), d = ta(t.tracePropagation), f = {
		db: $($({}, o), n),
		auth: $($({}, s), r),
		realtime: $($({}, c), i),
		storage: {},
		global: $($($({}, l), a), {}, { headers: $($({}, l?.headers ?? {}), a?.headers ?? {}) }),
		tracePropagation: {
...
var sa = class extends Ti {
	...
	constructor(e, t, n) {
		...
		Xi(t), ia(n), this.realtimeUrl = new URL("realtime/v1", r), this.realtimeUrl.protocol = this.realtimeUrl.protocol.replace("http", "ws"), this.authUrl = new URL("auth/v1", r), this.storageUrl = new URL("storage/v1", r), this.functionsUrl = new URL("functions/v1", r);
		let i = `sb-${r.hostname.split(".")[0]}-auth-token`, a = {
			db: Mi,
			realtime: Pi,
			auth: $($({}, Ni), {}, { storageKey: i }),
			global: ji,
			tracePropagation: Fi
		}, o = aa(n ?? {}, a);
		...
		} })) : this.auth = this._initSupabaseAuthClient(o.auth ?? {}, this.headers, o.global.fetch), this.fetch = Zi(t, e, this._getSessionToken.bind(this), o.global.fetch, o.tracePropagation), this.functionsFetch = Zi(t, e, this._getSessionToken.bind(this), o.global.fetch, o.tracePropagation, { omitApiKeyAsBearer: !0 }), this.realtime = this._initRealtimeClient($({
			headers: this.headers,
			accessToken: this._getAccessToken.bind(this),
			fetch: this.fetch
		}, o.realtime)), this.accessToken && Promise.resolve(this.accessToken()).then((e) => this.realtime.setAuth(e)).catch((e) => console.warn("Failed to set initial Realtime auth token:", e)), this.rest = new ue(new URL("rest/v1", r).href, {
			headers: this.headers,
			schema: o.db.schema,
			fetch: this.fetch,
	...
	_initRealtimeClient(e) {
		return new wt(this.realtimeUrl.href, $($({}, e), {}, { params: $($({}, { apikey: this.supabaseKey }), e?.params) }));
	}
...
ua() && console.warn("⚠️  Node.js 20 and below are deprecated and will no longer be supported in future versions of @supabase/supabase-js. Please upgrade to Node.js 22 or later. For more information, visit: https://github.com/orgs/supabase/discussions/45715");
//#endregion
...
var da = [
	...
	5e3,
	8e3
], ma = 1800, ha = 6e3, ga = Object.freeze([
	"offline",
	"timeout",
	"network",
	"server",
...
Object.freeze(["reconnecting", "paused"]);
var _a = new Set([
	408,
	425,
	502,
...
function va(e, t) {
	let n = Number(e?.context?.status) || void 0, r = String(e?.name ?? ""), i = "other", a = !1;
	if (r === "AuthRetryableFetchError") i = "network", a = !0;
	else if (r === "FunctionsFetchError") {
		let t = String(e?.context?.name ?? "");
		i = t === "AbortError" || t === "TimeoutError" ? "timeout" : "network", a = !0;
	} else r === "FunctionsRelayError" ? (i = "server", a = !0) : n === 408 || n === 504 ? (i = "timeout", a = !0) : n && _a.has(n) ? (i = "server", a = !0) : n && n >= 500 && (i = "server");
	return t?.code === "SERVER_UNAVAILABLE" && (i = "server", a = !0), {
		kind: i,
		transient: a,
		status: n,
		code: t?.code
...
function ya(e, t = !0) {
	return t === !1 || e === "offline" ? "You seem to be offline. Your farm is safe and opens again as soon as you are back." : e === "timeout" ? "The connection is slow right now. Your farm is safe; we keep trying." : e === "server" ? "The farm servers are busy for a moment. Your farm is safe; we keep trying." : e === "network" ? "We could not reach your farm. Your farm is safe; we keep trying." : "Your farm is safe. Reconnect to continue.";
}
var ba = (e, t = !0) => t === !1 ? "offline" : ga.includes(e?.kind) ? e.kind : "other";
function xa(e) {
	let t = e?.operation;
	return t === "action" ? typeof e.requestId == "string" && e.requestId.length > 0 : t === "family" ? Object.keys(e).length === 1 : [
		"load",
		"player_search",
		"player_profile",
		"rename",
		"avatar",
		"admin_online",
		"admin_recent_players",
		"admin_retention"
	].includes(t);
}
var Sa = (e) => new Promise((t) => setTimeout(t, e));
async function Ca(e, { repeatable: t = !0, delays: n = da, budget: r = fa, sleep: i = Sa, now: a = Date.now, onRetry: o } = {}) {
	let s = a();
	for (let c = 0;; c++) try {
		return await e(c);
	} catch (e) {
		let l = n[c];
		if (!t || !e?.transient || l === void 0 || a() - s + l > r) throw e;
		o?.(e, c + 1), await i(l);
	}
}
function wa({ probe: e, onStatus: t, onRecovered: n = () => {}, track: r = () => {}, timers: i = globalThis, now: a = Date.now, isOnline: o = () => !0, isHidden: s = () => !1 }) {
	let c = "ok", l = 0, u = 0, d = null, f = null, p = !1, m = 0, h = "other", g = () => {
		d !== null && (i.clearTimeout(d), d = null);
	}, ee = () => {
		f !== null && (i.clearTimeout(f), f = null);
	}, _ = (e) => {
		c = e, t(e, { reason: h });
	}, v = (e) => {
		g(), d = i.setTimeout(() => {
	...
	}, y = () => pa[Math.min(u, pa.length - 1)];
	function b() {
		c === "reconnecting" && a() - l >= 6e4 && (_("paused"), r("connection_problem", {
			reason: h,
			stage: "paused"
		}));
	}
	...
	function ne() {
		let e = c;
		g(), m++, c = "ok", u = 0, r("connection_recovered", {
			reason: h,
			stage: e
		}), t("ok", { reason: h }), n(e);
	}
	function re(e = "other") {
		h = ga.includes(e) ? e : "other", c === "ok" && (l = a(), u = 0, _("reconnecting"), r("connection_problem", {
			reason: h,
			stage: "reconnecting"
		}), v(y()));
	}
	function ie(e = "other") {
		h = ga.includes(e) ? e : "other", c !== "paused" && (c === "ok" && (l = a(), u = 0), _("paused"), r("connection_problem", {
			reason: h,
			stage: "paused"
		}), v(y()));
	}
...
var Ta = "https://jnmdirvidffzxukbdmij.supabase.co"?.trim(), Ea = "sb_publishable_ZOdMOklo0mbfEscPeRy6QA_SRLcssxc"?.trim(), Da = !!(Ta && Ea), Oa = Ta ? `${Ta.replace(/\/$/, "")}/functions/v1` : null, ka = Da ? la(Ta, Ea, { auth: {
	persistSession: !0,
	autoRefreshToken: !0,
	detectSessionInUrl: !0,
	storageKey: "harvest-tycoon:auth"
} }) : null, Aa = (e) => typeof e == "string" && /^[A-Za-z0-9][A-Za-z0-9 _-]{2,19}$/.test(e.trim());
function ja(e) {
...
async function Ma() {
	let { data: { session: e }, error: t } = await ka.auth.getSession();
	if (t) throw t;
	if (!e || e.user.is_anonymous) return null;
	let n = await ka.auth.getUser();
	if (n.error) {
		if (n.error.status === 401 || n.error.status === 403) return null;
		let { kind: e, transient: t } = va(n.error);
		throw Object.assign(n.error, {
			kind: e,
			transient: t
		});
	}
...
async function Na(e) {
	let { data: t, error: n } = await ka.functions.invoke("farm-api", {
		body: e,
		timeout: 2e4
	});
	if (n) {
		...
		let { kind: t, transient: r, status: i, code: a } = va(n, e), o = Error(e?.error || ya(t, globalThis.navigator?.onLine));
...
function Pa(e, { retry: t = !0 } = {}) {
	return Ca(() => Na(e), { repeatable: t && xa(e) });
}
async function Fa(e) {
	let { data: t, error: n } = await ka.functions.invoke("diamond-checkout", {
		body: e,
		timeout: 2e4
	});
	if (n) {
...
//#endregion
export { Fa as a, Ma as c, wa as d, ba as f, Da as i, ma as l, Pa as n, ka as o, Oa as r, Aa as s, ja as t, ya as u };
