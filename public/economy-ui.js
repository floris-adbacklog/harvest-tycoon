...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily,onPlace}){
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   const cost=upgradeCost(state,key),estate=upgradeRequirements(state,key),next=bs.level+1;
   ...
   // One row: what the upgrade costs (coins and the building's own goods), the Upgrade button and exactly what is still missing.
   ...
   content+=`<div class="upgrade-panel"><span class="upgrade-icon"><i data-lucide="circle-fading-arrow-up"></i></span><div><strong>${cost?`Upgrade to level ${next}`:'Fully upgraded'}</strong><p>${upgradeNote}</p></div>${cost===null?'':`<div class="upgrade-payments">${coinRow}</div>`}</div>`;
  }
