...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily,onPlace}){
 ...
 function renderBuilding(){
  ...
  $('building-content').querySelectorAll('[data-clear-planting]').forEach(btn=>btn.addEventListener('click',async()=>{
   // Removing a tree or bush throws its fruit away: the same red confirmation as leaving a family.
   const p=state.plots[Number(btn.dataset.clearPlanting)];if(!p?.crop)return;
   if(!await confirmAction({title:`Remove your ${CROPS[p.crop].name.toLowerCase()}?`,description:`This clears field ${p.id+1} and discards any fruit. Replanting costs ${number(seedCost(state,p.crop))} coins.`,confirmLabel:'Remove planting',cancelLabel:'Keep it',picture:p.crop,tone:'danger'}))return;
   mutate(async()=>{await runAction({type:'clear_planting',id:Number(btn.dataset.clearPlanting),expectedPlantedAt:Number(btn.dataset.plantedAt)});onExpand();return 'Planting removed. Your field is ready for a new crop.';});
  }));
