...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily}){
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   const upgradeNote=cost?`${state.boosts.upgradeCredits?'Your 50% upgrade voucher is included in this price. ':''}${productionSlots(next)} simultaneous batches, plus ${Math.round(productionSpeed(next)*100)}% shorter production time than level 1. ${estate?'An estate upgrade for a farm with many fields. ':''}${jobs.length?'Collect all current batches first.':'Applies to your next batches.'}`:`Level ${MAX_BUILDING_LEVEL}: ${slots} simultaneous batches; production takes ${Math.round(productionSpeed(MAX_BUILDING_LEVEL)*100)}% less time.`;
