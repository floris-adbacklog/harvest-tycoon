...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily}){
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   const upgradeNote=cost?`${state.boosts.upgradeCredits?'Your 50% upgrade voucher is included in this price. ':''}${productionSlots(next,key)} simultaneous ${productionSlots(next,key)===1?'batch':'batches'}${productionSlots(next,key)>slots?' (one more)':''}, plus ${Math.round(productionSpeed(next,key)*100)}% shorter production time than level 1. ${estate?'An estate upgrade for a farm with many fields. ':''}${jobs.length?'Your current batch keeps going; this applies from your next one.':'Applies to your next batches.'}`:`Level ${MAX_BUILDING_LEVEL}: ${slots} simultaneous batches; production takes ${Math.round(productionSpeed(MAX_BUILDING_LEVEL,key)*100)}% less time.`;
