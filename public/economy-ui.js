...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily,onPlace}){
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   // Each way to pay is one row: what it costs, its own Upgrade button and what is still missing for exactly that way.
   const diamondsOpen=diamondCost!==null&&featureUnlocked(state,'boosts');
   const missingGoods=estate?Object.entries(estate.materials).filter(([item,n])=>(state.inventory[item]??0)<n).map(([item,n])=>`${number(n-(state.inventory[item]??0))} ${ITEMS[item].name}`):[];
   const coinsShort=cost!==null&&state.coins<cost?[`${number(cost-state.coins)} coins`]:[],stillNeeded=[...missingGoods,...coinsShort];
   const coinRow=cost===null?'':`<div class="upgrade-option"><div class="upgrade-cost"><span class="upgrade-price">${art('coins')}<b>${number(cost)}</b></span>${estate?`<span class="upgrade-plus">+</span><div class="ingredients expansion-materials">${itemList(estate.materials,true)}</div>`:''}</div><button id="upgrade-building" class="small-button" ${mutating||stillNeeded.length||!estateLevelOk?'disabled':''} aria-label="Upgrade with coins${estate?' and goods':''}">Upgrade</button>${estate&&!estateLevelOk?`<small class="shortfall">Level ${next} unlocks at farm level ${estate.level}. You are level ${levelOf(state)}.</small>`:stillNeeded.length?`<small class="shortfall">Still needed: ${stillNeeded.join(' and ')}.</small>`:''}</div>`;
   const diamondRow=cost===null||!diamondsOpen?'':`<div class="upgrade-option is-diamonds${state.diamonds>=diamondCost?' is-affordable':''}"><div class="upgrade-cost"><span class="upgrade-price">${art('diamonds')}<b>${number(diamondCost)}</b></span></div><button id="upgrade-building-diamonds" class="small-button diamond-option" ${mutating||state.diamonds<diamondCost?'disabled':''} aria-label="Upgrade with ${number(diamondCost)} diamonds">Upgrade</button>${state.diamonds<diamondCost?`<small class="shortfall">You need ${number(diamondCost-state.diamonds)} more diamonds.</small>`:''}</div>`;
   content+=`<div class="upgrade-panel"><span class="upgrade-icon"><i data-lucide="circle-fading-arrow-up"></i></span><div><strong>${cost?`Upgrade to level ${next}`:'Fully upgraded'}</strong><p>${upgradeNote}</p></div>${cost===null?'':`<div class="upgrade-payments">${coinRow}${diamondRow}</div>`}</div>`;
  }
  ...
  // Diamonds are asked once more (26 Sep 2026): the diamond row sits right under the coin row, so a quick tap could spend hundreds.
  $('upgrade-building-diamonds')?.addEventListener('click',async()=>{
   const price=diamondUpgradeCost(state,key),level=bs.level;
   const sure=await confirmAction({title:`Upgrade the ${b.name} with diamonds?`,description:`Are you sure you want to spend ${number(price)} diamonds to upgrade the ${b.name} to level ${level+1}?`,confirmLabel:`Spend ${number(price)} diamonds`,cancelLabel:'Keep my diamonds',picture:'diamonds'});
   if(!sure)return;
   mutate(async()=>{const r=await runAction({type:'upgrade',building:key,currency:'diamonds',expectedCost:price,expectedLevel:level});return `${b.name} upgraded to level ${r.level} for ${r.cost} diamonds.`;});
  });
