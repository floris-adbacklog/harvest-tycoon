...
import {sellableStock,keptStock,rookieLeft,marketSaleValue,vipActive,buildingCost,constructionNeeds,recipeUnlocked,diamondUpgradeCost,itemAvailable,recipeUnlockHint,guidedFarm,buildingEligible,buildingUnlockHint,BUILDING_LEVELS,cropUnlockHint,featureUnlocked,CROPS,PRODUCTS,ITEMS,BUILDINGS,RECIPES,MAX_PLOTS,recipeAvailability,upgradeCost,expansionCost,expansionLevel,seedCost,formatDuration,cropDuration,recipeDuration,productionSpeed,MAX_BUILDING_LEVEL,upgradeRequirements,expansionMaterials,productionSlots,productionJobs,recipeValue,marketQuote,marketHighlights,utcDay,levelOf,cropUnlocked,buildingUnlocked,FACTORY_UPGRADE_MULTIPLIER} from './farm-state.js';
...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily}){
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   const upgradeNote=cost?`${state.boosts.upgradeCredits?'Your 50% upgrade voucher is included in this price. ':''}${productionSlots(next,key)} simultaneous ${productionSlots(next,key)===1?'batch':'batches'}${productionSlots(next,key)>slots?' (one more)':''}, plus ${Math.round(productionSpeed(next,key)*100)}% shorter production time than level 1. ${estate?'An estate upgrade for a farm with many fields. ':''}${key==='factory'?`Costs ${FACTORY_UPGRADE_MULTIPLIER}× a regular building's upgrade — one batch here is worth many elsewhere. `:''}${jobs.length?'Your current batch keeps going; this applies from your next one.':'Applies to your next batches.'}`:`Level ${MAX_BUILDING_LEVEL}: ${slots} simultaneous batches; production takes ${Math.round(productionSpeed(MAX_BUILDING_LEVEL,key)*100)}% less time.`;
  ...
  function updateBatch(output){
   ...
   card.querySelector('.recipe-value').textContent=RECIPES[id].coins?`Costs ${number(RECIPES[id].coins*count)} coins · the goods are worth about ${number(value.output*count)} coins on the market.`:`Today’s total: ingredients ${number(value.input*count)} coins → goods ${number(value.output*count)} coins · ${signed(value.added*count)} coins from processing`;
 ...
 function renderMarket(){
  ...
  $('market-items').innerHTML=entries.map(([key,c])=>{const q=marketQuote(key,now),stock=sellableStock(state,key,now),kept=keptStock(state,key,now),quantity=stock?Math.min(stock,Math.max(1,sellQuantities[key]??1)):0;sellQuantities[key]=quantity;return `<div class="market-row dynamic-market-row">${itemArt(key)}<div class="market-item-copy"><strong>${c.name}</strong><span class="market-current-price">${number(marketSaleValue(state,q.price,now))} <small>coins each${vipActive(state,now)?' · VIP +5%':''}${multiplier===2?' · 2× boost':''}</small></span><span class="market-price-context">Normal ${number(q.normal)} · Range ${number(q.min)}–${number(q.max)}</span><span class="demand-pill ${q.demand}">${q.label} · ${signed(q.change)}%</span></div><div class="market-sale-controls"><label for="sell-quantity-${key}"><span>${kept?`${number(stock)} to sell · ${number(kept)} kept, free in ${rookieTimeLeft(rookieLeft(state,now))}`:`${number(stock)} in stock`}</span><output id="sell-count-${key}">${quantity} selected</output></label><input id="sell-quantity-${key}" data-sell-range="${key}" type="range" min="${stock?1:0}" max="${stock}" step="1" value="${quantity}" aria-label="Quantity of ${c.name} to sell" ${!stock||marketSelling?'disabled':''}><div class="market-sale-buttons"><button class="small-button" data-sell="${key}" ${!stock||marketSelling?'disabled':''}>Sell ${number(quantity)} · ${number(marketSaleValue(state,quantity*q.price,now))} coins</button><button class="small-button" data-sell-item-all="${key}" ${!stock||marketSelling?'disabled':''}>Sell all</button></div></div></div>`;}).join('');
