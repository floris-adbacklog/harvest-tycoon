import {CROPS,PRODUCTS,ITEMS,BUILDINGS,RECIPES,MAX_PLOTS,recipeAvailability,upgradeCost,expansionCost,seedCost,formatDuration,cropDuration,recipeDuration,productionSpeed,MAX_BUILDING_LEVEL,expansionMaterials,cropIcon} from './farm-state.js';
...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,sound,runAction,onEstate}){
 ...
 function itemArt(key){return CROPS[key]?`<img draggable="false" src="${cropIcon(key)}" style="filter:${CROPS[key].filter??'none'}" alt="">`:`<span class="product-art ${PRODUCTS[key].color}"><i data-lucide="${PRODUCTS[key].icon}"></i></span>`;}
 ...
 function chooseCrop(key){selectedCrop=key;$('selected-crop-image').src=cropIcon(key);$('selected-crop-image').style.filter=CROPS[key].filter??'none';$('selected-crop-name').textContent=CROPS[key].name;$('selected-crop-price').textContent=`${seedCost(state,key)} · ${seconds(cropDuration(state,key))}`;onCrop(key);}
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   content+=`<div class="upgrade-panel"><span class="upgrade-icon"><i data-lucide="circle-fading-arrow-up"></i></span><div><strong>${cost?`Upgrade to level ${bs.level+1}`:'Fully upgraded'}</strong><p>${cost?`${Math.round(productionSpeed(bs.level+1)*100)}% shorter production time than level 1. ${bs.job?'Collect the current batch first.':'Applies to your next batches.'}`:`Level ${MAX_BUILDING_LEVEL,expansionMaterials,cropIcon}: production takes ${Math.round(productionSpeed(MAX_BUILDING_LEVEL)*100)}% less time.`}</p></div><button id="upgrade-building" class="small-button" ${cost===null||state.coins<cost||bs.job?'disabled':''}>${cost?`${cost} coins`:'Max level'}</button></div>`;
  }
