...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate,onFamily}){
 ...
 function renderBuilding(){
  ...
  function updateBatch(output){
   ...
   card.querySelector('.recipe-value').innerHTML=RECIPES[id].coins?`Costs ${number(RECIPES[id].coins*count)} coins · worth about ${number(value.output*count)}`:`<strong>${signed(value.added*count)} coins</strong> more than the ingredients`;
