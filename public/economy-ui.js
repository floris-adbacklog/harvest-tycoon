...
export function createEconomyUI({state,onChange,onCrop,onExpand,notify,runAction,onEstate}){
 ...
 function status(key,now=farmNow()){
  ...
  if(!buildingUnlocked(state,key))return {text:levelOf(state)<BUILDINGS[key].minLevel?`Unlocks at level ${BUILDINGS[key].minLevel}`:`Open for ${number(BUILDINGS[key].buildCost)} coins`,kind:'locked'};
 ...
 function chooseCrop(key){if(!cropUnlocked(state,key)){notify(`Reach level ${CROPS[key].minLevel} to plant ${CROPS[key].name}.`);return;}selectedCrop=key;$('selected-crop-art').innerHTML=art(key);$('selected-crop-name').textContent=CROPS[key].name;$('selected-crop-price').textContent=`${seedCost(state,key)} · ${seconds(cropDuration(state,key))}`;onCrop(key);}
 function renderSeeds(){
  ...
  $('crop-catalog').innerHTML=Object.entries(CROPS).sort((a,b)=>a[1].duration-b[1].duration).filter(([,c])=>seedFilter==='all'||(seedFilter==='quick'?c.duration<=900000:seedFilter==='later'?c.duration>900000&&c.duration<28800000:c.duration>=28800000)).map(([key,c])=>`<button class="crop-card crop-${key} ${selectedCrop===key?'selected':''}" data-choose-crop="${key}" ${cropUnlocked(state,key)?'':'disabled'} aria-pressed="${selectedCrop===key}"><div class="crop-art"><span class="crop-pace">${c.perennial?'Grows back':c.duration<=900000?'Quick grow':c.duration<28800000?'Day crop':'Slow grow'}</span>${art(key,'crop-picture')}${selectedCrop===key?'<span class="crop-selected"><i data-lucide="check"></i></span>':''}</div><strong>${c.name}</strong><span class="crop-stats"><span>${art('coins','tiny-coin')} ${seedCost(state,key)}</span><span><i data-lucide="clock-3"></i> ${seconds(cropDuration(state,key))}</span></span><small>${marketQuote(key,farmNow()).price} coins today · 1–3 yield</small><span class="crop-use">${cropUnlocked(state,key)?c.use:`Unlocks at level ${c.minLevel}`}</span>${c.perennial?`<small class="perennial-note">Plant once · First harvest ${seconds(cropDuration(state,key))}<br>Then ${seconds(cropDuration(state,key,true))} after each collection.<br>One harvest waits; no offline stockpiling.</small>`:''}</button>`).join('');
 ...
 function renderBuilding(){
  ...
  if(key==='farmhouse'){
   ...
   const eligible=levelOf(state)>=b.minLevel;
