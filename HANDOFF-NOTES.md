...
Installed app (PWA) layout (2026-09-21): the game frame `#farm-host` must stay `inset:0`. An earlier version inset it by `env(safe-area-inset-*)`, but the game inside the iframe reads the same safe areas itself (iOS shows them to same-origin frames), so they were counted twice: black bars above and below and sheets that stopped short of the bottom. The game now reads the bottom inset through one variable, `--safe-bottom` (`public/styles.css`; identical to the browser value). `public/app-mode.js` (loaded in `play.html` and `farm.html`) sets `html[data-app-mode=standalone]` only in the installed app and measures `--viewport-shortfall`: some iOS versions report a layout height a status bar short of the screen, which leaves a strip at the bottom where nothing can be tapped. `public/pwa-layout.css` (game frame) then takes that strip off the bottom inset and adds a soft veil under the status bar; `welcome.css` paints the strip in the colour of the bottom bar. Everything is scoped to the installed app, so the browser layout is untouched. Not verified on a real iPhone (no Xcode simulator here): if the bottom bar still looks off, ask for a screenshot and the value of `--viewport-shortfall` from the Safari Web Inspector. Tests: `tests/pwa-layout.test.mjs`. Update 2026-09-26: the user's iPhone showed that strip as an empty cream band under the farm. In that case the page now stretches `#farm-host` over the strip (`bottom:calc(0px - var(--viewport-shortfall))`, `welcome.css`). The frame then measures no gap of its own and gets the page's strip as `--frame-strip`, so `--safe-bottom` keeps its buttons above the strip in case it takes no taps. Measured on the user's iPhone (admin dashboard, Settings, "This device"): screen 402×874, window and fixed box 812, status bar 62. The stretched fixed frame was 874 tall, but the strip stayed cream: iOS draws no fixed element below the short layout. So in that case (`html[data-viewport-short]`) `#farm-host` is `position:absolute` with height `100% + shortfall`. A visualViewport listener scrolls the page back to the top after the keyboard. The installed app also keeps the game viewport from the start (`app-mode.js`, `src/page-zoom.js`): changing it at sign-in may be what triggers the quirk. Still to confirm on the iPhone.
...
render of the farm itself from a steeper angle (`shootMinimap` in game.js renders into a corner of the screen buffer with a
scissor, copies it, and draws the normal view again in the same frame; fog off for it), taken when the farm opens, after the
