...
await build({configFile:false,publicDir:false,define:{'import.meta.env.VITE_SUPABASE_URL':JSON.stringify(url),'import.meta.env.VITE_SUPABASE_ANON_KEY':JSON.stringify(key)},build:{outDir:'public/cloud',emptyOutDir:true,lib:{entry:{cloud:'src/main.js','game-cloud':'src/game-cloud.js'},formats:['es'],fileName:(_format,name)=>name+'.js'},minify:true,sourcemap:false}});
