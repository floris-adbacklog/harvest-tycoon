export async function createStarterPackUI(bridge){
 const {CROPS,STARTER_PACK_CROPS,formatDuration}=await import(/* @vite-ignore */ '/farm-state.js');
 ...
 const chip=document.createElement('button');chip.id='starter-pack-chip';chip.className='icon-button';chip.hidden=true;chip.type='button';chip.title='Starter Pack';chip.setAttribute('aria-label','Starter Pack, €2.99');chip.innerHTML='<img src="/assets/icons/starter-pack.svg" alt=""><small>€2.99</small>';
 ...
 async function refresh(){if(refreshing||disposed)return;refreshing=true;try{const data=await bridge.payments({operation:'catalog'});if(disposed)return;catalog=data;offset=data.serverNow-Date.now();render();}catch{if(!catalog)button.hidden=chip.hidden=true;}finally{refreshing=false;}}
