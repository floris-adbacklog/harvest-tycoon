...
export function prefsFromRow(row){
 ...
 return {pushCrops:row.push_crops===true,pushProduction:row.push_production===true,pushDaily:row.push_daily===true,emailDigest:row.email_digest===true,digestHour:validHour(row.digest_hour)?row.digest_hour:DEFAULT_PREFS.digestHour};
}
export function paramsFromPrefs(prefs,timezone=browserTimezone()){
 const hour=Number(prefs.digestHour);
 return {p_push_crops:prefs.pushCrops===true,p_push_production:prefs.pushProduction===true,p_push_daily:prefs.pushDaily===true,p_email_digest:prefs.emailDigest===true,p_digest_hour:validHour(hour)?hour:DEFAULT_PREFS.digestHour,p_timezone:timezone};
}
...
export function createNotifications(supabase,{configUrl=null,fetchImpl=globalThis.fetch,timezone=browserTimezone,win=globalThis.window}={}){
 ...
 return {
  ...
  async save(prefs){
   ...
   return {pushCrops:params.p_push_crops,pushProduction:params.p_push_production,pushDaily:params.p_push_daily,emailDigest:params.p_email_digest,digestHour:params.p_digest_hour};
  }
 };
}
