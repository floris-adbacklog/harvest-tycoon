-- Daily family sharing with every crop and good. The limits stay exactly as they were (level 10, 48 hours on the farm,
-- 24 hours in the family, 3 sent and 3 received per kind a day, once per pair, one request a day), and everything still
-- moves from one farm to the other without creating anything. What changes: a gift and a request can be 1–5 of any
-- crop or good instead of only 3 wheat / wheat, corn or feed. The list below is Object.keys(ITEMS) from farm-state.js
-- (tests/family-sharing.test.mjs keeps them equal); a store that sends a gift without an item still gets 3 wheat.
-- Based on the live definition of harvest_social read on 23 Sep 2026; only the item and quantity rules and the reply
-- differ. Run after retention-social.sql.
alter table public.family_social_requests drop constraint if exists family_social_requests_item_check;
alter table public.family_social_requests add constraint family_social_requests_item_check check (item ~ '^[a-z]{2,24}$');
...
declare
 ...
 items constant text[]:=array['corn','wheat','cabbage','pumpkin','sunflower','barley','lettuce','redcabbage','cauliflower','greenbeans','apples','berries','honey','grainmeal','fertilizer','salad','pickles','flour','feed','oil','milk','eggs','cheese','bread','pie','vegetables','applejuice','applepie','berrypreserves','berrytart','stew','orchardjuice','berrysmoothie','applecompote','applevinegar','pickledbeans','beangratin','orchardsalad','berrycheesecake','harvesthamper'];
begin
