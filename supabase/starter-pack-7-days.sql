-- The Starter Pack is there for 7 days instead of 72 hours (25 Sep 2026). Checkout (game/payments.js STARTER_WINDOW) records the
-- deadline on the purchase; this check only accepts a Starter Pack bought within that window. Offers that are already open are counted
-- from the same moment (reaching level 14, where diamond boosts unlock), so they run to 7 days too. The rest is the live function.
