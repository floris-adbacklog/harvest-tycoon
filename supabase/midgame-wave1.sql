-- Midgame expansion, wave 1 (levels 28–47): squash, pole beans, cider apples, squash soup, beeswax, wool, yarn, cloth and cider.
-- 1. Family sharing: the new crops and goods can be gifted and requested like every other item. Only the items list differs
--    from family-sharing-all-items.sql (the live definition read on 23 Sep 2026); limits, checks and the reply are unchanged.
--    tests/family-sharing.test.mjs keeps this list equal to Object.keys(ITEMS).
-- 2. Leaderboards: the three new crops count toward "crops harvested" and the six new goods toward "goods produced".
--    Crops grown in the Glasshouse are not field harvests and are not counted as goods either (as before for any crop).
--    Based on the live harvest_public_metrics read on 23 Sep 2026; nothing else changes and harvest_commit_farm is untouched.
-- Run after family-sharing-all-items.sql and pantry-production-counters.sql.
