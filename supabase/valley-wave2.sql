-- Wave 2 (levels 54-70): cherries, goat milk, goat cheese, beeswax candles, wool blankets, cherry jam and cherry pie.
-- 1. Family sharing: the seven new crops and goods can be gifted and requested like every other item. Only the items list differs
--    from midgame-wave1.sql; limits, checks and the reply are unchanged. tests/family-sharing.test.mjs keeps this list equal to
--    Object.keys(ITEMS).
-- 2. Leaderboards: cherries count toward "crops harvested" and the six new goods toward "goods produced". The cherries column
--    itself (harvested_cherries) came with midgame-crop-columns.sql. Nothing else changes; harvest_commit_farm is untouched.
-- Run after midgame-wave1.sql and midgame-crop-columns.sql.
