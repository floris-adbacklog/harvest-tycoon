import {normalizeFarm,levelOf} from './farm-state.js';

// The only account that may ever give coins, XP or diamonds to another farmer. Checked against the authenticated,
// server-verified user (index.ts already resolved this from a real Supabase JWT) — never against anything the
...
const MAX_GRANT=1000000,MAX_DIAMONDS=5000,MAX_MESSAGE=200;
export const validGrantAmount=(value,max)=>Number.isInteger(value)&&value>=0&&value<=max;
export function validGrant({coins,xp,diamonds}){
 return validGrantAmount(coins,MAX_GRANT)&&validGrantAmount(xp,MAX_GRANT)&&validGrantAmount(diamonds,MAX_DIAMONDS)&&Boolean(coins||xp||diamonds);
}
...
export async function handleAdminGrant({admin,body,user}){
 ...
 if(!validGrant({coins,xp,diamonds}))return respond({error:'Enter whole, non-negative amounts (at least one), within the per-gift limit.'},400);
 ...
 for(let attempt=0;attempt<5;attempt++){
  ...
  if(notify)state.pendingGift={coins,xp,diamonds,message,at:now};
  ...
  if(saved.data){
   const logged=await admin.from('admin_grants').insert({player_id:playerId,granted_by:user.id,coins,xp,diamonds,notified:notify,message});
   if(logged.error)throw logged.error;
   return respond({granted:{coins,xp,diamonds},totals:{coins:state.coins,xp:state.xp,diamonds:state.diamonds,level}});
  }
 }
