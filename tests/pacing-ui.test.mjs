...
function harness({level=2,waitMinutes=30,kind='off',available=true,push=true,stored=null,enable='on',prefs={pushCrops:false,pushProduction:false,pushDaily:true,emailDigest:false,digestHour:8}}={}){
 ...
 const nudge=createReminderNudge({state,farmNow:()=>now,level:()=>level,notify:m=>log.toasts.push(m),track:(e,p)=>log.events.push([e,p.action])});
...
test('a production batch that will take a while also counts as waiting',async()=>{
 ...
 const n=createReminderNudge({state,farmNow:()=>now,level:()=>3,notify(){},track:()=>{}});await n.check();assert.equal(h.shown(),true);h.cleanup();
});
test('Not now closes it for good',async()=>{
 ...
 const again=createReminderNudge({state:{plots:[{crop:'corn',readyAt:Date.now()+3600000}],buildings:{}},farmNow:()=>Date.now(),level:()=>5,notify(){}});await again.check();assert.equal(h.shown(),false,'not even in a later session');h.cleanup();
});
