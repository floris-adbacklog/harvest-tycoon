...
test('twenty starter quests are appended after the old ones: small, unique and built on counters the game already keeps',()=>{
 ...
 assert.equal(STARTER_QUESTS.first,130);assert.equal(starters.length,STARTER_QUESTS.count);assert.equal(starters.length,20);assert.equal(QUESTS.length,204);
