...
test('all four hands-on jobs reward once, give useful goods and complete one round',()=>{
 ...
 assert.equal(ACTIVE_STATIONS.apiary.itemCount,3);assert.equal(ACTIVE_STATIONS.greenhouse.itemCount,3);for(const [id,a] of Object.entries(ACTIVE_STATIONS))if(!['apiary','greenhouse'].includes(id))assert.equal(a.itemCount??1,1,id);
...
test('client-supplied reward fields are ignored and XP boosts apply once',()=>{
 ...
 assert.equal(result.coins,0);assert.equal(result.xp,96);assert.equal(s.activities.completed.workshop,1);
});
