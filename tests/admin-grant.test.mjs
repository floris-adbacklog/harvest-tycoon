...
test('a waiting gift is picked up and cleared on the farmer\'s own next load, alongside level/chapter rewards',()=>{
 const code=readFileSync(new URL('../supabase/functions/farm-api/index.ts',import.meta.url),'utf8');
 const load=code.slice(code.indexOf("body.operation==='load'"),code.indexOf("body.operation==='load'")+1000);
 ...
 assert.match(load,/if\(levelReward\.levels\.length\|\|chapterReward\.chapters\.length\|\|gift\)\{/,'a waiting gift alone is enough to trigger the commit, like a level or chapter reward');
 assert.match(load,/return reply\(\{state,profile:\{\.\.\.profile,currency:state\.coins\},levelReward,chapterReward,gift,revision:row\.revision\+1,serverNow:now\}\);/);
});
