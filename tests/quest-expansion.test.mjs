...
test('new quests append to old IDs and keep beginner and claimed progress',()=>{
 ...
 assert.equal(QUESTS.length,173);assert.equal(QUESTS[31].title,'A lifelong grower');assert.equal(QUESTS[40].title,'Pumpkin perfection');
...
test('daily rotation covers accessible orders without locked chores or parallel goals for starters',()=>{
 assert.equal(DAILY_POOLS.flat().length,55);assert.equal(ORDER_POOL.length,44);
