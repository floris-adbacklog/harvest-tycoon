...
export function createActivitiesUI({state,runAction,notify,onResult}){
 ...
 async function act(action){
  ...
  try{
   ...
   if(result.finished){feedback=`Job complete! +${result.coins} coins · +${result.xp} XP${result.item?` · +${result.itemCount??1} ${ITEMS[result.item].name}`:''}${result.roundComplete?' · Farm round bonus included!':''}`;notify(feedback);}
