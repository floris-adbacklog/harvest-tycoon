...
export function createBoostsUI({state,runAction,onChange,notify}){
 ...
 function render(){
  ...
  $('boost-catalog').querySelectorAll('[data-buy-boost]').forEach(button=>button.onclick=()=>{
   try{
    const id=button.dataset.buyBoost,result=runAction({type:'buy_boost',boost:id});onChange();render();
