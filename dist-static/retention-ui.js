...
export function createRetentionUI({state,runAction,onChange,notify,getCrop,itemList}){
 ...
 function gift(){
  ...
  $('daily-gift').innerHTML=`<section class="gift-panel"><div class="gift-heading"><span class="gift-icon"><i data-lucide="gift"></i></span><div><h3>${claimed?'A little gift, just for you':'Good to see you, farmer!'}</h3><p>${streak?`${streak} day${streak===1?'':'s'} in a row · `:''}Best streak: ${state.login.best} days</p></div><span class="streak-pill"><i data-lucide="flame"></i> ${streak}</span></div><div class="streak-days">${DAILY_REWARDS.map((reward,i)=>`<div class="streak-day ${i===day?'current':''} ${i<day||claimed&&i===day?'collected':''}"><small>Day ${i+1}</small><i data-lucide="${i<day||claimed&&i===day?'check':i===6?'gift':'coins'}"></i><strong>${reward} <span>coins</span></strong><span class="streak-diamonds">${art('diamonds')} ${DAILY_DIAMONDS[i]}</span></div>`).join('')}</div><button id="checkin-gift" class="primary-button" ${claimed?'disabled':''}>${claimed?'Gift collected · see you tomorrow!':`Collect day ${day+1} gift · ${DAILY_REWARDS[day]} coins + ${DAILY_DIAMONDS[day]} diamonds`}<i data-lucide="${claimed?'check':'gift'}"></i></button><small class="gift-note">A new seven-day cycle follows day 7. Miss a day? Start again with your farm intact.</small></section>`;
