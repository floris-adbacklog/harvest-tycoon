...
function decorate(){
 ...
 addBuilding('windmill',13,4,{height:7.4});
 // Both the mill body and its moving sails are original parts from the supplied pack.
 const sail=cloneModel('tower_020',0,0,{height:6.5});scene.remove(sail);
 sail.position.set(0,-3.25,0);windmillRotor=new THREE.Group();windmillRotor.position.set(13,5.1,6.0);windmillRotor.add(sail);windmillRotor.userData.building='windmill';scene.add(windmillRotor);
 ...
 cloneModel('greenhouse_003',8,-18,{width:4.2,rotation:Math.PI/2});
 ...
 addUtility('stall','stall_002',-9.3,-5.7,{width:3.6,rotation:.15});
 cloneModel('prop_023',-7.3,-5.5,{width:.8});
 ...
 const trees=[[-17,-14,6],[-20,-10,5],[-19,1,4.5],[-18.8,6,4.7],[-17.4,8.5,4],[-18,12,6.2],[-17,17,5.5],[-5,19,5.8],[7,21,6],[14,15,5.4],[19,8,6],[21,1,5.7],[20,-10,6],[19,-19,6.1],[4,-21,5.4],[-10,-19,6.5],[-2,-22,7],[-23,7,6.5],[24,15,6.4],[-25,-1,6.4],[25,-17,7]];
...
function updateUI(){
 ...
 $('quest-total').textContent=QUESTS.length;updateHint();economy?.refresh();retention?.refresh();growth?.refresh();boosts?.refresh();quests?.refresh();mobileUI?.refresh();
}
...
function resize(){
 ...
 const mobile=mobileLayout.matches,span=(mobile?Math.max(15,17/aspect):37)/zoom;
 ...
 const focus=mobile?new THREE.Vector3(1.8+pan+panDepth,0,fieldCenter-pan+panDepth):new THREE.Vector3(pan+panDepth,0,1.8-pan+panDepth);
 ...
 $('zoom-in').disabled=zoom>=1.85;$('zoom-out').disabled=zoom<=.65;
...
function zoomFarm(value){zoom=Math.max(.65,Math.min(1.85,value));resize();}
function resetView(){zoom=1;pan=0;panDepth=0;resize();if(ready)updateUI();}
...
function bindUI(){
 ...
 $('farm-button').addEventListener('click',()=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());resetView();toast('Your fields, front and center.');});
 ...
 const toggleQuest=()=>{if(mobileLayout.matches){quests.open();return;}const hidden=!$('quest-body').hidden;$('quest-body').hidden=hidden;$('quest-collapse').setAttribute('aria-expanded',String(!hidden));$('quest-collapse').setAttribute('aria-label',hidden?'Expand quest':'Collapse quest');$('quest-collapse').innerHTML=`<i data-lucide="${hidden?'clipboard-check':'chevron-up'}"></i>`;icons();};
 $('quest-collapse').addEventListener('click',toggleQuest);
 document.querySelector('.quest-heading')?.addEventListener('click',event=>{if(event.target.closest('#quest-collapse'))return;if(mobileLayout.matches)quests.open();});
 ...
 $('zoom-in').addEventListener('click',()=>zoomFarm(zoom+.15));$('zoom-out').addEventListener('click',()=>zoomFarm(zoom-.15));$('zoom-reset').addEventListener('click',resetView);
