...
//#endregion
//#region node_modules/.pnpm/@supabase+functions-js@2.116.0/node_modules/@supabase/functions-js/dist/module/helper.js
...
//#endregion
//#region node_modules/.pnpm/@supabase+functions-js@2.116.0/node_modules/@supabase/functions-js/dist/module/FunctionsClient.js
var u = class {
	...
	invoke(e) {
		return r(this, arguments, void 0, function* (e, t = {}) {
			...
			try {
				...
				if (ne && ne === "true") throw new s(_);
				if (!_.ok) throw new c(_);
				let v = (_.headers.get("Content-Type") ?? "text/plain").split(";")[0].trim().toLowerCase(), re;
				return re = v === "application/json" ? yield _.json() : v === "application/octet-stream" || v === "application/pdf" ? yield _.blob() : v === "text/event-stream" ? _ : v === "multipart/form-data" ? yield _.formData() : yield _.text(), {
					data: re,
					error: null,
					response: _
				};
...
async function ae(e, t, n, r) {
	...
	for (;;) {
		...
		try {
			...
			if (r && i < f) {
				let e = p(i);
				i++, await re(e, n.signal);
				continue;
			}
			throw e;
		}
		if (ie(n.method, o.status, i, r)) {
			let e = o.headers?.get("Retry-After") ?? null, t = e === null ? p(i) : Math.max(0, parseInt(e, 10) || 0) * 1e3;
			await o.text(), i++, await re(t, n.signal);
			continue;
		}
		return o;
	}
...
var oe = class {
	...
	then(e, t) {
		...
		let r = this.fetch, i = (async () => {
			...
			let t = await ae(r, n.url.toString(), {
				method: n.method,
				headers: e,
	...
	async processResponse(e) {
		...
		if (e.ok) {
			...
			if (s && c && c.length > 1 && (i = parseInt(c[1])), t.isMaybeSingle && Array.isArray(r)) if (r.length > 1) {
				...
				}, r = null, i = null, a = 406, o = "Not Acceptable", t.shouldThrowOnError) throw new d(v(v({}, n), {}, { hint: n.hint ?? "" }));
...
}, ce = /* @__PURE__ */ RegExp("[,()]"), le = class extends se {
	throwOnError() {
		return super.throwOnError();
	}
	eq(e, t) {
	...
	in(e, t) {
		let n = Array.from(new Set(t)).map((e) => typeof e == "string" && ce.test(e) ? `"${e}"` : `${e}`).join(",");
	...
	select(e, t) {
		...
		return s.searchParams.set("select", o), r && c.append("Prefer", `count=${r}`), new le({
			method: i,
			url: s,
			headers: c,
			schema: this.schema,
	...
	update(e, { count: t } = {}) {
		...
		return t && r.append("Prefer", `count=${t}`), new le({
			method: "PATCH",
			url: n,
			headers: r,
			schema: this.schema,
	...
	delete({ count: e } = {}) {
		...
		return e && n.append("Prefer", `count=${e}`), new le({
			method: "DELETE",
			url: t,
			headers: n,
			schema: this.schema,
...
var pe = class e {
	...
	from(e) {
		...
		return new ue(new URL(`${this.url}/${e}`), {
			headers: new Headers(this.headers),
	...
	async getOpenApiSpec() {
		...
		try {
			...
			return fe(e, i.status, i.statusText);
		}
		if (i.ok) try {
			return {
				success: !0,
	...
	rpc(e, t = {}, { head: n = !1, get: r = !1, count: i } = {}) {
		...
		return l ? u.set("Prefer", i ? `count=${i},return=minimal` : "return=minimal") : i && u.set("Prefer", `count=${i}`), new le({
			method: a,
			url: o,
			headers: u,
			schema: this.schemaName,
...
}, he = "realtime-js/2.116.0", ge = "1.0.0", _e = "2.0.0", ve = _e, ye = 1e4, be = 15e3, xe = 1e4, y = {
	closed: "closed",
	errored: "errored",
	joined: "joined",
	joining: "joining",
...
var Te = (e, t, n = {}) => {
	let r = n.skipTypes ?? [];
	return t ? Object.keys(t).reduce((n, i) => (n[i] = Ee(i, e, t, r), n), {}) : {};
}, Ee = (e, t, n, r) => {
	let i = t.find((t) => t.name === e)?.type, a = n[e];
	return i && !r.includes(i) ? De(i, a) : Oe(a);
}, De = (e, t) => {
	if (e.charAt(0) === "_") return Me(t, e.slice(1, e.length));
	switch (e) {
		case b.bool: return ke(t);
		case b.float4:
		case b.float8:
		case b.int2:
		case b.int4:
		case b.int8:
		case b.numeric:
		case b.oid: return Ae(t);
		case b.json:
		case b.jsonb: return je(t);
		case b.timestamp: return Ne(t);
		case b.abstime:
		case b.date:
		case b.daterange:
		case b.int4range:
	...
	if (e[0] === "{" && r === "}") {
		...
		return r.map((e) => De(t, e));
	}
	return e;
}, Ne = (e) => typeof e == "string" ? e.replace(" ", "T") : e, Pe = (e) => {
	let t = new URL(e);
...
}, Fe = (e) => typeof e == "function" ? e : function() {
	return e;
}, Ie = typeof self < "u" ? self : null, Le = typeof window < "u" ? window : null, x = Ie || Le || globalThis, Re = "2.0.0", ze = 1e4, Be = 1e3, Ve = 100, S = {
	connecting: 0,
	open: 1,
	closing: 2,
	closed: 3
	...
	longpoll: "longpoll",
	websocket: "websocket"
}, Ue = { complete: 4 }, We = "base64url.bearer.phx.", Ge = class {
	constructor(e, t, n, r) {
	...
	constructor(e, t, n) {
		this.state = C.closed, this.topic = e, this.params = Fe(t || {}), this.socket = n, this.bindings = [], this.bindingRef = 0, this.timeout = this.socket.timeout, this.joinedOnce = !1, this.joinPush = new Ge(this, w.join, this.params, this.timeout), this.pushBuffer = [], this.stateChangeRefs = [], this.rejoinTimer = new Ke(() => {
			...
			this.socket.hasLogger() && this.socket.log("channel", `timeout ${this.topic}`, this.joinPush.timeout), new Ge(this, w.leave, Fe({}), this.timeout).send(), this.state = C.errored, this.joinPush.reset(), this.socket.isConnected() && this.rejoinTimer.scheduleTimeout();
		}), this.on(w.reply, (e, t) => {
	...
	static xhrRequest(e, t, n, r, i, a, o, s) {
		...
		return e.onerror = () => s && s(null), e.onreadystatechange = () => {
			e.readyState === Ue.complete && s && s(this.parseJSON(e.responseText));
	...
	constructor(e, t) {
		t && t.length === 2 && t[1].startsWith(We) && (this.authToken = atob(t[1].slice(We.length))), this.endPoint = null, this.token = null, this.skipHeartbeat = !0, this.reqs = /* @__PURE__ */ new Set(), this.awaitingBatchAck = !1, this.currentBatch = null, this.currentBatchTimer = null, this.batchBuffer = [], this.onopen = function() {}, this.onerror = function() {}, this.onmessage = function() {}, this.onclose = function() {}, this.pollEndpoint = this.normalizeEndpoint(e), this.readyState = S.connecting, setTimeout(() => this.poll(), 0);
	}
	normalizeEndpoint(e) {
		return e.replace("ws://", "http://").replace("wss://", "https://").replace(RegExp("(.*)/" + He.websocket), "$1/" + He.longpoll);
	}
	endpointURL() {
	...
	send(e) {
		typeof e != "string" && (e = Ye(e)), this.currentBatch ? this.currentBatch.push(e) : this.awaitingBatchAck ? this.batchBuffer.push(e) : (this.currentBatch = [e], this.currentBatchTimer = setTimeout(() => {
			this.batchSend(this.currentBatch), this.currentBatch = null;
		}, 0));
	}
	batchSend(e, t = 0) {
		this.awaitingBatchAck = !0;
		let n = t + Ve, r = e.slice(t, n);
	...
	ajax(e, t, n, r, i) {
		let a;
		a = Je.request(e, this.endpointURL(), t, n, this.timeout, () => {
	...
	constructor(e, t = {}) {
		this.stateChangeCallbacks = {
			...
			error: [],
			message: []
		}, this.channels = [], this.sendBuffer = [], this.ref = 0, this.fallbackRef = null, this.timeout = t.timeout || ze, this.transport = t.transport || x.WebSocket || Xe, this.conn = void 0, this.primaryPassedHealthCheck = !1, this.longPollFallbackMs = t.longPollFallbackMs, this.fallbackTimer = null;
		let n = null;
		try {
			n = x && x.sessionStorage;
		} catch {}
		this.sessionStore = t.sessionStorage || n, this.establishedConnections = 0, this.defaultEncoder = Qe.encode.bind(Qe), this.defaultDecoder = Qe.decode.bind(Qe), this.closeWasClean = !0, this.disconnecting = !1, this.binaryType = t.binaryType || "arraybuffer", this.connectClock = 1, this.pageHidden = !1, this.encode = void 0, this.decode = void 0, this.transport === Xe ? (this.encode = this.defaultEncoder, this.decode = this.defaultDecoder) : (this.encode = t.encode || this.defaultEncoder, this.decode = t.decode || this.defaultDecoder);
		let r = null;
		Le && Le.addEventListener && (Le.addEventListener("pagehide", (e) => {
			this.conn && (this.disconnect(), r = this.connectClock);
		}), Le.addEventListener("pageshow", (e) => {
			r === this.connectClock && (r = null, this.connect());
		}), Le.addEventListener("visibilitychange", () => {
			...
			console.log(`${e}: ${t}`, n);
		}), this.longpollerTimeout = t.longpollerTimeout || 2e4, this.params = Fe(t.params || {}), this.endPoint = `${e}/${He.websocket}`, this.vsn = t.vsn || Re, this.heartbeatTimeoutTimer = null, this.heartbeatTimer = null, this.heartbeatSentAt = null, this.pendingHeartbeatRef = null, this.reconnectTimer = new Ke(() => {
			if (this.pageHidden) {
		...
		}, this.reconnectAfterMs), this.authToken = t.authToken && Fe(t.authToken);
	}
	...
	endPointURL() {
		let e = Je.appendParams(Je.appendParams(this.endPoint, this.params()), { vsn: this.vsn });
	...
	connect(e) {
		e && (console && console.log("passing params to connect is deprecated. Instead pass :params to the Socket constructor"), this.params = Fe(e)), !(this.conn && !this.disconnecting) && (this.longPollFallbackMs && this.transport !== Xe ? this.connectWithFallback(Xe, this.longPollFallbackMs) : this.transportConnect());
	}
	log(e, t, n) {
	...
	heartbeatTimeout() {
		if (this.pendingHeartbeatRef) {
			...
			this.triggerChanError(/* @__PURE__ */ Error("heartbeat timeout")), this.closeWasClean = !1, this.teardown(() => this.reconnectTimer.scheduleTimeout(), Be, "heartbeat timeout");
		}
	}
	resetHeartbeat() {
	...
	constructor(t, n) {
		let r = rt(n);
		this.presence = new Ze(t.getChannel(), r), this.presence.onJoin((n, r, i) => {
	...
	static transformState(e) {
		return e = nt(e), Object.getOwnPropertyNames(e).reduce((t, n) => {
			let r = e[n];
			return t[n] = tt(r), t;
		}, {});
	}
...
var ot = class {
	...
	constructor(e, t) {
		this.channel = e, this.presenceAdapter = new et(this.channel.channelAdapter, t);
	}
};
//#endregion
...
var ut = /[,()"\\]/, dt = (e) => ut.test(e) || e !== e.trim(), ft = (e) => `"${e.replace(/\\/g, "\\\\").replace(/"/g, "\\\"")}"`, pt = (e) => {
	let t = e === null ? "null" : String(e);
	return dt(t) ? ft(t) : t;
}, mt = (e) => e === null ? "null" : String(e), ht = (e, t) => {
	if (e === "in") {
		let e = Array.isArray(t) ? t : [t];
		if (e.length === 0) throw Error("Realtime `in` filter requires at least one value.");
		return `in.(${Array.from(new Set(e)).map((e) => pt(e)).join(",")})`;
	}
	return e === "is" ? `is.${mt(t)}` : `${e}.${pt(t)}`;
}, gt = class {
	constructor() {
		this.filters = [];
	}
	add(e, t, n, r = !1) {
		let i = r ? "not." : "";
		return this.filters.push(`${e}=${i}${ht(t, n)}`), this;
	}
	eq(e, t) {
...
var yt = class e {
	...
	constructor(e, t = { config: {} }, n) {
		if (this.topic = e, this.params = t, this.socket = n, this.bindings = {}, this.subTopic = e.replace(/^realtime:/i, ""), this.params.config = Object.assign({
			...
			private: !1
		}, t.config), this.channelAdapter = new ct(this.socket.socketAdapter, e, this.params), this.presence = new ot(this), this._onClose(() => {
			this.socket._remove(this);
		}), this._updateFilterTransform(), this.broadcastEndpointURL = Pe(this.socket.socketAdapter.endPointURL()), this.private = this.params.config.private || !1, !this.private && this.params.config?.broadcast?.replay) throw Error(`tried to use replay on public channel '${this.topic}'. It must be a private channel.`);
	}
	subscribe(e, t = this.timeout) {
		if (this.socket.isConnected() || this.socket.connect(), this.channelAdapter.isClosed()) {
			let { config: { broadcast: n, presence: r, private: i, postgres_changes_options: a } } = this.params, o = this.bindings.postgres_changes?.map((e) => e.filter) ?? [], s = !!this.bindings[vt.PRESENCE] && this.bindings[vt.PRESENCE].length > 0 || this.params.config.presence?.enabled === !0, c = {}, l = Object.assign({
				broadcast: n,
	...
	on(e, t, n) {
		let r = this.channelAdapter.isJoined() || this.channelAdapter.isJoining(), i = e === vt.PRESENCE || e === vt.POSTGRES_CHANGES;
	...
	_on(t, n, r) {
		...
		if ((a instanceof gt || typeof a == "object" && a && typeof a.build == "function") && (n = Object.assign(Object.assign({}, n), { filter: a.build() })), i === vt.POSTGRES_CHANGES && this.bindings[i]?.find((t) => e.isSamePostgresFilter(t.filter, n))) return this.socket.log("error", `duplicate \`postgres_changes\` binding for ${this.topic} ignored`, n), this;
	...
	_notThisChannelEvent(e, t) {
		let { close: n, error: r, leave: i, join: a } = Se;
		return t && [
			n,
			r,
			i,
	...
	_getPayloadRecords(e) {
		...
		return (e.type === "INSERT" || e.type === "UPDATE") && (t.new = Te(e.columns, e.record)), (e.type === "UPDATE" || e.type === "DELETE") && (t.old = Te(e.columns, e.old_record)), t;
	}
}, bt = class {
	constructor(e, t) {
		this.socket = new $e(e, t);
	}
	get timeout() {
		return this.socket.timeout;
	}
	...
	isConnecting() {
		return this.socket.connectionState() == Ce.connecting;
	}
	isDisconnecting() {
		return this.socket.connectionState() == Ce.closing;
	}
	connectionState() {
...
var Et = "\n  addEventListener(\"message\", (e) => {\n    if (e.data.event === \"start\") {\n      setInterval(() => postMessage({ event: \"keepAlive\" }), e.data.interval);\n    }\n  });", Dt = class {
	...
	constructor(e, t) {
		if (this.channels = [], this.accessTokenValue = null, this.accessToken = null, this.apiKey = null, this.httpEndpoint = "", this.headers = {}, this.params = {}, this.ref = 0, this.serializer = new we(), this._manuallySetToken = !1, this._authPromise = null, this._authGeneration = 0, this._workerHeartbeatTimer = void 0, this._pendingWorkerHeartbeatRef = null, this._pendingDisconnectTimer = null, this._disconnectOnEmptyChannelsAfterMs = 0, this._resolveFetch = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), !t?.params?.apikey) throw Error("API key is required to connect to Realtime");
		this.apiKey = t.params.apikey;
		let n = this._initializeOptions(t);
		this.socketAdapter = new bt(e, n), this.httpEndpoint = Pe(e), this.fetch = this._resolveFetch(t?.fetch);
	}
	connect() {
	...
	connectionState() {
		return this.socketAdapter.connectionState() || Ce.closed;
	}
	isConnected() {
	...
	channel(e, t = { config: {} }) {
		...
		if (r) return r;
		{
			let n = new yt(`realtime:${e}`, t, this);
	...
	async _performAuth(e, t) {
		...
		t === this._authGeneration && (this.accessToken ? this._manuallySetToken = !1 : r && (this._manuallySetToken = !0), this.accessTokenValue != n && (this.accessTokenValue = n, this.channels.forEach((e) => {
			...
			e.updateJoinPayload(t), e.joinedOnce && e.channelAdapter.isJoined() && e.channelAdapter.push(Se.access_token, { access_token: n });
		})));
	}
	...
	_workerObjectUrl(e) {
		...
		if (e) t = e;
			...
			let e = new Blob([Et], { type: "application/javascript" });
	...
	_initializeOptions(e) {
		...
		let t = {};
		t.timeout = e?.timeout ?? ye, t.heartbeatIntervalMs = e?.heartbeatIntervalMs ?? xt.HEARTBEAT_INTERVAL, this._disconnectOnEmptyChannelsAfterMs = e?.disconnectOnEmptyChannelsAfterMs ?? 2 * (e?.heartbeatIntervalMs ?? xt.HEARTBEAT_INTERVAL), t.transport = e?.transport ?? me.getWebSocketConstructor(), t.params = e?.params, t.logger = e?.logger, t.heartbeatCallback = this._wrapHeartbeatCallback(e?.heartbeatCallback), t.sessionStorage = e?.sessionStorage ?? Tt(), t.reconnectAfterMs = e?.reconnectAfterMs ?? ((e) => St[e - 1] || Ct);
		let n, r, i = e?.vsn ?? ve;
		switch (i) {
			case ge:
...
function jt(e) {
	...
	return { async request({ method: n, path: r, query: i, body: a, headers: o }) {
		let s = kt(e.baseUrl, r, i), c = await At(e.auth), l = await t(s, {
			method: n,
			headers: {
		...
		if (!l.ok) {
			...
			throw new Ot(t?.message ?? `Request failed with status ${l.status}`, {
				status: l.status,
				icebergType: t?.type,
				icebergCode: t?.code,
				details: e
...
var Nt = class {
	...
	async listNamespaces(e) {
		let t = e ? { parent: Mt(e.namespace) } : void 0;
	...
	async dropNamespace(e) {
		await this.client.request({
			method: "DELETE",
			path: `${this.prefix}/namespaces/${Mt(e.namespace)}`
		});
	}
	...
	async namespaceExists(e) {
		try {
			...
			if (e instanceof Ot && e.status === 404) return !1;
			throw e;
		}
	}
	async createNamespaceIfNotExists(e, t) {
		try {
			...
			if (e instanceof Ot && e.status === 409) return;
			throw e;
		}
	}
};
...
var Ft = class {
	...
	async listTables(e) {
		return (await this.client.request({
			method: "GET",
			path: `${this.prefix}/namespaces/${Pt(e.namespace)}/tables`
		})).data.identifiers;
	}
	...
	async updateTable(e, t) {
		let n = await this.client.request({
			method: "POST",
			path: `${this.prefix}/namespaces/${Pt(e.namespace)}/tables/${e.name}`,
			body: t
		});
		return {
	...
	async createTableIfNotExists(e, t) {
		try {
			...
			if (n instanceof Ot && n.status === 409) return await this.loadTable({
				namespace: e.namespace,
				name: t.name
			});
			throw n;
	...
	constructor(e) {
		...
		this.client = jt({
			...
			auth: e.auth,
			fetchImpl: e.fetch
		}), this.accessDelegation = e.accessDelegation?.join(","), this.namespaceOps = new Nt(this.client, t), this.tableOps = new Ft(this.client, t, this.accessDelegation);
	}
	async listNamespaces(e) {
...
var Jt = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), Yt = (e) => {
	if (typeof e != "object" || !e) return !1;
	let t = Object.getPrototypeOf(e);
	return (t === null || t === Object.prototype || Object.getPrototypeOf(t) === null) && !(Symbol.toStringTag in e) && !(Symbol.iterator in e);
}, Xt = (e) => {
	if (Array.isArray(e)) return e.map((e) => Xt(e));
...
}, Zt = (e) => !e || typeof e != "string" || e.length === 0 || e.length > 100 || e.trim() !== e || e.includes("/") || e.includes("\\") ? !1 : /^[\w!.\*'() &$@=;:+,?-]+$/.test(e), Qt = (e) => e.split("/").map(encodeURIComponent).join("/"), $t = (e) => {
	...
	if (typeof e == "object" && e && "json" in e && typeof e.json == "function") {
		...
		Number.isFinite(i) || (i = 500), n.json().then((e) => {
			...
			t(new Wt($t(e), i, n, r, e?.code));
		}).catch(() => {
			let e = i + "";
			t(new Wt(n.statusText || `HTTP ${i} error`, i, e, r));
		});
	} else t(new Gt($t(e), e, r));
}, tn = (e, t, n, r) => {
	let i = {
		method: e,
		headers: t?.headers || {}
	...
	if (Yt(r)) {
		...
		i.headers = Kt(e, "Content-Type", n ?? "application/json"), i.body = JSON.stringify(r);
	} else i.body = r;
	return t?.duplex && (i.duplex = t.duplex), E(E({}, i), n);
};
async function nn(e, t, n, r, i, a, o) {
	return new Promise((s, c) => {
		e(n, tn(t, r, i, a)).then((e) => {
			if (!e.ok) throw e;
			...
			return e.json();
		}).then((e) => s(e)).catch((e) => en(e, c, r, o));
	});
}
function rn(e = "storage") {
	return {
		get: async (t, n, r, i) => nn(t, "GET", n, r, i, void 0, e),
		post: async (t, n, r, i, a) => nn(t, "POST", n, i, a, r, e),
		put: async (t, n, r, i, a) => nn(t, "PUT", n, i, a, r, e),
		head: async (t, n, r, i) => nn(t, "HEAD", n, E(E({}, r), {}, { noResolveJson: !0 }), i, void 0, e),
		remove: async (t, n, r, i, a) => nn(t, "DELETE", n, i, a, r, e)
	};
}
var { get: an, post: D, put: on, head: sn, remove: cn } = rn("storage"), O = rn("vectors"), ln = class {
	constructor(e, t = {}, n, r = "storage") {
		this.shouldThrowOnError = !1, this.url = e, this.headers = qt(t), this.fetch = Jt(n), this.namespace = r;
	}
	throwOnError() {
		return this.shouldThrowOnError = !0, this;
	}
	setHeader(e, t) {
		return this.headers = Kt(this.headers, e, t), this;
	}
	...
	constructor(e, t) {
		this.downloadFn = e, this.shouldThrowOnError = t, this[un] = "StreamDownloadBuilder", this.promise = null;
	}
	then(e, t) {
...
}, fn = Symbol.toStringTag, pn = class {
	constructor(e, t) {
		this.downloadFn = e, this.shouldThrowOnError = t, this[fn] = "BlobDownloadBuilder", this.promise = null;
	}
	asStream() {
		return new dn(this.downloadFn, this.shouldThrowOnError);
	}
	then(e, t) {
		return this.getPromise().then(e, t);
	}
	...
	async uploadOrUpdate(e, t, n, r) {
		...
		return i.handleOperation(async () => {
			let a, o = E(E({}, hn), r), s = E(E({}, i.headers), e === "POST" && { "x-upsert": String(o.upsert) }), c = o.metadata;
			...
			let l = i._removeEmptyFolders(t), u = i._getFinalPath(l), d = await (e == "PUT" ? on : D)(i.fetch, `${i.url}/object/${u}`, a, E({ headers: s }, o?.duplex ? { duplex: o.duplex } : {}));
			return {
				path: l,
				id: d.Id,
				fullPath: d.Key
	...
	async uploadToSignedUrl(e, t, n, r) {
		...
		return s.searchParams.set("token", t), i.handleOperation(async () => {
			let e, t = E(E({}, hn), r), o = E(E({}, i.headers), { "x-upsert": String(t.upsert) }), c = t.metadata;
			...
			return {
				path: a,
				fullPath: (await on(i.fetch, s.toString(), e, E({ headers: o }, t?.duplex ? { duplex: t.duplex } : {}))).Key
			};
		});
	}
	async createSignedUploadUrl(e, t) {
		...
		return n.handleOperation(async () => {
			...
			if (!s) throw new Ht("No token returned by API");
			return {
	...
	download(e, t, n) {
		...
		return new pn(() => an(this.fetch, `${this.url}/${r}/${o}${a ? `?${a}` : ""}`, {
			headers: this.headers,
			noResolveJson: !0
	...
	async info(e, t) {
		...
		let a = i.toString();
		return n.handleOperation(async () => Xt(await an(n.fetch, `${n.url}/object/info/${r}${a ? `?${a}` : ""}`, { headers: n.headers })));
	}
	async exists(e) {
		var t = this;
		let n = t._getFinalPath(e);
		try {
			return await sn(t.fetch, `${t.url}/object/${n}`, { headers: t.headers }), {
				data: !0,
				error: null
			};
		} catch (e) {
			if (t.shouldThrowOnError) throw e;
			if (Ut(e)) {
				let t = e instanceof Wt ? e.status : e instanceof Gt ? e.originalError?.status : void 0;
	...
	async remove(e) {
		var t = this;
		return t.handleOperation(async () => await cn(t.fetch, `${t.url}/object/${t.bucketId}`, { prefixes: e }, { headers: t.headers }));
	}
	async purgeCache(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = Qt(r._getFinalPath(e)), a = new URLSearchParams();
			t?.transformations && a.set("transformations", "true");
			let o = a.toString();
			return await cn(r.fetch, `${r.url}/cdn/${i}${o ? `?${o}` : ""}`, {}, { headers: r.headers }, n);
		});
	}
	async list(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = t?.sortBy ? E(E({}, mn.sortBy), t.sortBy) : mn.sortBy, a = E(E(E({}, mn), t), {}, {
				sortBy: i,
				prefix: e || ""
			});
...
}, _n = { "X-Client-Info": "storage-js/2.116.0" }, vn = class extends ln {
	constructor(e, t = {}, n, r) {
		let i = new URL(e);
		...
		let a = i.href.replace(/\/$/, ""), o = E(E({}, _n), t);
		super(a, o, n, "storage");
	}
	async listBuckets(e) {
		var t = this;
		return t.handleOperation(async () => {
			...
			return await an(t.fetch, `${t.url}/bucket${n}`, { headers: t.headers });
		});
	}
	async getBucket(e) {
		var t = this;
		return t.handleOperation(async () => await an(t.fetch, `${t.url}/bucket/${e}`, { headers: t.headers }));
	}
	...
	async updateBucket(e, t) {
		var n = this;
		return n.handleOperation(async () => await on(n.fetch, `${n.url}/bucket/${e}`, {
			id: e,
			name: e,
			public: t.public,
			file_size_limit: t.fileSizeLimit,
	...
	async deleteBucket(e) {
		var t = this;
		return t.handleOperation(async () => await cn(t.fetch, `${t.url}/bucket/${e}`, {}, { headers: t.headers }));
	}
	async getBucketLifecycle(e) {
		var t = this;
		return t.handleOperation(async () => await an(t.fetch, t.bucketLifecycleUrl(e), { headers: t.headers }));
	}
	async updateBucketLifecycle(e, t) {
		var n = this;
		return n.handleOperation(async () => await on(n.fetch, n.bucketLifecycleUrl(e), t, { headers: n.headers }));
	}
	async deleteBucketLifecycle(e) {
		var t = this;
		return t.handleOperation(async () => await cn(t.fetch, t.bucketLifecycleUrl(e), {}, { headers: t.headers }));
	}
	async purgeBucketCache(e, t, n) {
		...
		return r.handleOperation(async () => {
			...
			let a = i.toString();
			return await cn(r.fetch, `${r.url}/cdn/${Qt(e)}${a ? `?${a}` : ""}`, {}, { headers: r.headers }, n);
		});
	}
	bucketLifecycleUrl(e) {
		return `${this.url}/bucket/${Qt(e)}/lifecycle`;
	}
	...
	async listBuckets(e) {
		...
		return t.handleOperation(async () => {
			...
			return await an(t.fetch, i, { headers: t.headers });
		});
	}
	async deleteBucket(e) {
		var t = this;
	...
	from(e) {
		var t = this;
		if (!Zt(e)) throw new Ht("Invalid bucket name: File, folder, and bucket names must follow AWS object key naming guidelines and should avoid the use of any other characters.");
		let n = new It({
			baseUrl: this.url,
			catalogName: e,
			auth: {
	...
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = E(E({}, _n), {}, { "Content-Type": "application/json" }, t);
		super(r, i, n, "vectors");
	}
	async createIndex(e) {
		var t = this;
	...
	index(e) {
		return new Tn(this.url, this.headers, this.vectorBucketName, e, this.fetch);
	}
}, Tn = class extends xn {
	...
	get vectors() {
		return new Cn(this.url + "/vector", {
			headers: this.headers,
			fetch: this.fetch
		});
	}
	get analytics() {
		return new yn(this.url + "/iceberg", this.headers, this.fetch);
	}
}, Dn = "2.116.0", On = 30 * 1e3, kn = 3 * On, An = 2 * On, jn = "http://localhost:9999", Mn = "supabase.auth.token", Nn = { "X-Client-Info": `gotrue-js/${Dn}` }, Pn = "X-Supabase-Api-Version", Fn = { "2024-01-01": {
	timestamp: Date.parse("2024-01-01T00:00:00.0Z"),
	name: "2024-01-01"
} }, In = /^([a-z0-9_-]{4})*($|[a-z0-9_-]{3}$|[a-z0-9_-]{2}$)$/i, Ln = "sb_flow_id", Rn = class extends Error {
	constructor(e, t, n) {
...
var P = () => typeof window < "u" && typeof document < "u", pr = {
	tested: !1,
	writable: !1
}, mr = () => {
	if (!P()) return !1;
	...
	if (pr.tested) return pr.writable;
	let e = `lswt-${Math.random()}${Math.random()}`;
	try {
		globalThis.localStorage.setItem(e, e), globalThis.localStorage.removeItem(e), pr.tested = !0, pr.writable = !0;
	} catch {
		pr.tested = !0, pr.writable = !1;
	}
	return pr.writable;
};
function hr(e) {
...
var gr = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), _r = (e) => typeof e == "object" && !!e && "status" in e && "ok" in e && "json" in e && typeof e.json == "function", F = async (e, t, n) => {
...
function yr(e) {
	...
	for (let e = 0; e < t.length; e++) if (!In.test(t[e])) throw new $n("JWT not in base64url format");
	return {
...
async function Mr(e, t, n, r, i) {
	...
	for (a.push(n); a.length > 5;) {
		let n = a.shift();
		await L(e, kr(t, n)), i?.(n);
	}
	await F(e, Ar(t), a), await F(e, `${t}-code-verifier`, r);
}
async function Nr(e, t, n) {
	if (n) {
		let r = await I(e, kr(t, n));
		return {
			verifier: typeof r == "string" ? r : null,
			flowId: n
		};
	}
	let r = await I(e, `${t}-code-verifier`);
	return {
		verifier: typeof r == "string" ? r : null,
		flowId: null
	};
...
async function R(e, t, n) {
	...
	let i = kr(t, n), a = await I(e, i);
	await L(e, i);
	let o = await jr(e, t), s = o.filter((e) => e !== n);
	s.length !== o.length && (s.length > 0 ? await F(e, Ar(t), s) : await L(e, Ar(t))), a != null && a === await I(e, r) && await L(e, r);
}
async function Pr(e, t) {
	let n = await jr(e, t);
	for (let r of n) await L(e, kr(t, r));
	await L(e, Ar(t)), await L(e, `${t}-code-verifier`);
}
function Fr(e, t) {
	...
	let o = r.includes("?") ? "&" : "?";
	return `${r}${o}${Ln}=${encodeURIComponent(t)}${i}`;
}
...
async function Jr(e) {
	if (!_r(e)) throw new Jn(Kr(e), 0);
	let t;
	try {
		t = await e.json();
	} catch (t) {
		throw qr.includes(e.status) ? new Jn(e.statusText || `HTTP ${e.status}`, e.status) : new A(Kr(t), t);
	}
	if (qr.includes(e.status)) throw new Jn(Kr(t), e.status);
	let n, r = Rr(e);
	if (r && r.getTime() >= Fn["2024-01-01"].timestamp && typeof t == "object" && t && typeof t.code == "string" ? n = t.code : typeof t == "object" && t && typeof t.error_code == "string" && (n = t.error_code), !n) {
		if (typeof t == "object" && t && typeof t.weak_password == "object" && t.weak_password && Array.isArray(t.weak_password.reasons) && t.weak_password.reasons.length && t.weak_password.reasons.reduce((e, t) => e && typeof t == "string", !0)) throw new Qn(Kr(t), e.status, t.weak_password.reasons);
	} else if (n === "weak_password") throw new Qn(Kr(t), e.status, t.weak_password?.reasons || []);
	else if (n === "session_not_found") throw new M();
	throw new zn(Kr(t), e.status || 500, n);
}
var Yr = (e, t, n, r) => {
	let i = {
		method: e,
...
async function V(e, t, n, r) {
	...
	i["X-Supabase-Api-Version"] || (i[Pn] = Fn["2024-01-01"].name), r?.jwt && (i.Authorization = `Bearer ${r.jwt}`);
	let a = r?.query ?? {};
...
function H(e) {
	let t = null;
	ti(e) && (t = Object.assign({}, e), e.expires_at || (t.expires_at = dr(e.expires_in)));
...
var ni = [
	...
	async generateLink(e) {
		try {
			...
			return "newEmail" in r && (i.new_email = r?.newEmail, delete i.newEmail), await V(this.fetch, "POST", `${this.url}/admin/generate_link`, {
				body: i,
				headers: this.headers,
				xform: $r,
				redirectTo: t?.redirectTo
	...
	async listUsers(e) {
		try {
			let t = {
				...
				lastPage: 0,
				total: 0
			}, n = await V(this.fetch, "GET", `${this.url}/admin/users`, {
				headers: this.headers,
				noResolveJson: !0,
				query: {
	...
	async _listFactors(e) {
		z(e.userId);
		try {
			let { data: t, error: n } = await V(this.fetch, "GET", `${this.url}/admin/users/${e.userId}/factors`, {
				headers: this.headers,
				xform: (e) => ({
					data: { factors: e },
					error: null
	...
	async _deleteFactor(e) {
		...
		try {
			return {
				data: await V(this.fetch, "DELETE", `${this.url}/admin/users/${e.userId}/factors/${e.id}`, { headers: this.headers }),
				error: null
			};
		} catch (e) {
			if (k(e)) return {
	...
	async _listOAuthClients(e) {
		try {
			let t = {
				...
				lastPage: 0,
				total: 0
			}, n = await V(this.fetch, "GET", `${this.url}/admin/oauth/clients`, {
				headers: this.headers,
				noResolveJson: !0,
				query: {
					page: (e?.page)?.toString() ?? "",
	...
	async _updateOAuthClient(e, t) {
		try {
			return await V(this.fetch, "PUT", `${this.url}/admin/oauth/clients/${e}`, {
				body: t,
				headers: this.headers,
				xform: (e) => ({
					data: e,
	...
	async _listCustomProviders(e) {
		try {
			let t = {};
			return e?.type && (t.type = e.type), await V(this.fetch, "GET", `${this.url}/admin/custom-providers`, {
				headers: this.headers,
				query: t,
				xform: (e) => ({
	...
	async _createCustomProvider(e) {
		try {
			return await V(this.fetch, "POST", `${this.url}/admin/custom-providers`, {
				body: e,
				headers: this.headers,
				xform: (e) => ({
					data: e,
	...
	async _getCustomProvider(e) {
		try {
			return await V(this.fetch, "GET", `${this.url}/admin/custom-providers/${e}`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _updateCustomProvider(e, t) {
		try {
			return await V(this.fetch, "PUT", `${this.url}/admin/custom-providers/${e}`, {
				body: t,
				headers: this.headers,
				xform: (e) => ({
					data: e,
	...
	async _deleteCustomProvider(e) {
		try {
			return await V(this.fetch, "DELETE", `${this.url}/admin/custom-providers/${e}`, {
				headers: this.headers,
				noResolveJson: !0
			}), {
				data: null,
	...
	async _adminListPasskeys(e) {
		B(this.experimental), z(e.userId);
		try {
			return await V(this.fetch, "GET", `${this.url}/admin/users/${e.userId}/passkeys`, {
				headers: this.headers,
				xform: (e) => ({
					data: e,
					error: null
	...
	async _adminDeletePasskey(e) {
		B(this.experimental), z(e.userId), z(e.passkeyId);
		try {
			return await V(this.fetch, "DELETE", `${this.url}/admin/users/${e.userId}/passkeys/${e.passkeyId}`, {
				headers: this.headers,
				noResolveJson: !0
			}), {
				data: null,
...
function hi(e) {
	...
	let { challenge: t, user: r, excludeCredentials: i } = e, a = n(e, [
		...
		"user",
		"excludeCredentials"
	]), o = lr(t).buffer, s = Object.assign(Object.assign({}, r), { id: lr(r.id).buffer }), c = Object.assign(Object.assign({}, a), {
		challenge: o,
		user: s
	});
	if (i && i.length > 0) {
...
function gi(e) {
	...
	let { challenge: t, allowCredentials: r } = e, i = n(e, ["challenge", "allowCredentials"]), a = lr(t).buffer, o = Object.assign(Object.assign({}, i), { challenge: a });
	if (r && r.length > 0) {
...
var Oi = class {
	...
	async _authenticate({ factorId: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		if (!t) return {
			data: null,
			error: new Rn("rpId is required for WebAuthn authentication")
		};
		try {
			if (!bi()) return {
				data: null,
	...
	async _register({ friendlyName: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		if (!t) return {
			data: null,
			error: new Rn("rpId is required for WebAuthn registration")
		};
		try {
			if (!bi()) return {
				data: null,
...
var ki = {
	...
	_logPrefix() {
		return `GoTrueClient@${this.storageKey}:${this.instanceID} (${Dn}) ${(/* @__PURE__ */ new Date()).toISOString()}`;
	}
	_debug(...e) {
	...
	async signInAnonymously(e) {
		try {
			let { data: t, error: n } = await V(this.fetch, "POST", `${this.url}/signup`, {
				headers: this.headers,
				body: {
					data: e?.options?.data ?? {},
	...
	async signUp(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				this.flowType === "pkce" && ([o, s, t] = await this._getCodeChallengeAndMethod()), n = await V(this.fetch, "POST", `${this.url}/signup`, {
					headers: this.headers,
				...
				n = await V(this.fetch, "POST", `${this.url}/signup`, {
					headers: this.headers,
					body: {
						phone: t,
						password: r,
			...
			if (i || !r) return await R(this.storage, this.storageKey, t), this._returnResult({
				data: {
					user: null,
					session: null
				},
			...
			if (await R(this.storage, this.storageKey, t), k(e)) return this._returnResult({
				data: {
					user: null,
					session: null
				},
	...
	async signInWithEthereum(e) {
		...
		try {
			let { data: r, error: i } = await V(this.fetch, "POST", `${this.url}/token?grant_type=web3`, {
				headers: this.headers,
				body: Object.assign({
					chain: "ethereum",
					message: t,
	...
	async _exchangeCodeForSession(e, t) {
		let n = t?.flowId != null, r = n ? Dr(t?.flowId) : P() ? Dr(hr(window.location.href)[Ln]) : null;
		...
		try {
			...
			let { data: t, error: n } = await V(this.fetch, "POST", `${this.url}/token?grant_type=pkce`, {
				headers: this.headers,
				body: {
					auth_code: e,
					code_verifier: o
			...
			if (await R(this.storage, this.storageKey, a), n) throw n;
			...
			if (await R(this.storage, this.storageKey, a), k(e)) return this._returnResult({
				data: {
					user: null,
					session: null,
					redirectType: null
	...
	async signInWithIdToken(e) {
		try {
			let { options: t, provider: n, token: r, access_token: i, nonce: a } = e, { data: o, error: s } = await V(this.fetch, "POST", `${this.url}/token?grant_type=id_token`, {
				headers: this.headers,
				body: {
					provider: n,
					id_token: r,
	...
	async signInWithOtp(e) {
		...
		try {
			if ("email" in e) {
				...
				let { error: o } = await V(this.fetch, "POST", `${this.url}/otp`, {
					headers: this.headers,
					body: {
						email: n,
						data: r?.data ?? {},
			...
			if ("phone" in e) {
				let { phone: t, options: n } = e, { data: r, error: i } = await V(this.fetch, "POST", `${this.url}/otp`, {
					headers: this.headers,
					body: {
						phone: t,
						data: n?.data ?? {},
	...
	async verifyOtp(e) {
		try {
			...
			let { data: r, error: i } = await V(this.fetch, "POST", `${this.url}/verify`, {
				headers: this.headers,
	...
	async signInWithSSO(e) {
		...
		try {
			...
			let i = await V(this.fetch, "POST", `${this.url}/sso`, {
	...
	async _reauthenticate() {
		try {
			return await this._useSession(async (e) => {
				...
				if (n) throw n;
				if (!t) throw new M();
				let { error: r } = await V(this.fetch, "GET", `${this.url}/reauthenticate`, {
					headers: this.headers,
					jwt: t.access_token
				});
				return this._returnResult({
	...
	async resend(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				let { error: c } = await V(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						email: r,
						type: i,
				...
				return c && await R(this.storage, this.storageKey, t), this._returnResult({
					data: {
						user: null,
						session: null
					},
				...
				let { phone: t, type: r, options: i } = e, { data: a, error: o } = await V(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						phone: t,
						type: r,
	...
	async __loadSession() {
		...
		try {
			let e = null, t = await I(this.storage, this.storageKey);
			...
			let n = e.expires_at ? e.expires_at * 1e3 - Date.now() < kn : !1;
			if (this._debug("#__loadSession()", `session has${n ? "" : " not"} expired`, "expires_at", e.expires_at), !n) {
				if (this.userStorage) {
					let t = await I(this.userStorage, this.storageKey + "-user");
			...
			if (i) {
				if (e.expires_at && e.expires_at * 1e3 > Date.now()) {
					let t = await I(this.storage, this.storageKey);
	...
	async _getUser(e) {
		try {
			...
			if (k(e)) return Vn(e) && await this._removeSession(), this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _updateUser(e, t = {}) {
		...
		try {
			return await this._useSession(async (r) => {
				...
				let { data: l, error: u } = await V(this.fetch, "PUT", `${this.url}/user`, {
					headers: this.headers,
			...
			if (await R(this.storage, this.storageKey, n), k(e)) return this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _getSessionFromURL(e, t) {
		try {
			if (!P()) throw new Wn("No browser detected.");
			...
			switch (t) {
				case "implicit":
					...
					break;
				case "pkce":
					if (this.flowType === "implicit") throw new Wn("Not a valid implicit grant flow url.");
					break;
				default:
			}
			if (t === "pkce") {
				if (this._debug("#_initialize()", "begin", "is PKCE flow", !0), !e.code) throw new Kn("No code detected.");
				let { data: t, error: n } = await this._exchangeCodeForSession(e.code, { flowId: e[Ln] });
				if (n) throw n;
				let r = new URL(window.location.href);
				return r.searchParams.delete("code"), r.searchParams.delete(Ln), window.history.replaceState(window.history.state, "", r.toString()), {
					data: {
						session: t.session,
	...
	async _isPKCECallback(e) {
		if (!e.code) return !1;
		let t = Dr(e[Ln]);
		return t && await I(this.storage, kr(this.storageKey, t)) ? !0 : !!await I(this.storage, `${this.storageKey}-code-verifier`);
	}
	...
	async _signOut({ scope: e } = { scope: "global" }) {
		return await this._useSession(async (t) => {
			...
			if (a) {
				...
				if (t && !(Bn(t) && (t.status === 404 || t.status === 401 || t.status === 403) || Vn(t))) return e !== "others" && await n(), this._returnResult({ error: t });
			}
	...
	async _emitInitialSession(e) {
		return await this._useSession(async (t) => {
			try {
				...
				if (await this.stateChangeEmitters.get(e)?.callback("INITIAL_SESSION", null), this._debug("INITIAL_SESSION", "callback id", e, "error", t), Zn(t)) return;
				Vn(t) || Yn(t) || Bn(t) && (t.code === "refresh_token_not_found" || t.code === "refresh_token_already_used" || t.code === "session_expired") ? console.warn(t) : console.error(t);
			}
		});
	}
	async resetPasswordForEmail(e, t = {}) {
		...
		try {
			...
			if (await R(this.storage, this.storageKey, i), k(e)) return this._returnResult({
				data: null,
				error: e
			});
			throw e;
	...
	async linkIdentityOAuth(e) {
		...
		try {
			let { data: n, error: r } = await this._useSession(async (n) => {
				...
				return t = o, await V(this.fetch, "GET", a, {
					headers: this.headers,
	...
	async linkIdentityIdToken(e) {
		return await this._useSession(async (t) => {
			try {
				...
				if (n) throw n;
				let { options: i, provider: a, token: o, access_token: s, nonce: c } = e, { data: l, error: u } = await V(this.fetch, "POST", `${this.url}/token?grant_type=id_token`, {
					headers: this.headers,
				...
				if (await R(this.storage, this.storageKey, null), k(e)) return this._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	async _refreshAccessToken(e) {
		...
		try {
			let n = Date.now();
			return await xr(async (n) => (n > 0 && await br(200 * 2 ** (n - 1)), this._debug(t, "refreshing attempt", n), await V(this.fetch, "POST", `${this.url}/token?grant_type=refresh_token`, {
	...
	async _recoverAndRefresh() {
		...
		try {
			...
			if (t && this.userStorage) {
				let e = await I(this.userStorage, this.storageKey + "-user");
				!this.storage.isServer && Object.is(this.storage, this.userStorage) && !e && (e = { user: t.user }, await F(this.userStorage, this.storageKey + "-user", e)), t.user = e?.user ?? Ur();
			} else if (t && !t.user && !t.user) {
				let e = await I(this.storage, this.storageKey + "-user");
				e && e?.user ? (t.user = e.user, await L(this.storage, this.storageKey + "-user"), await F(this.storage, this.storageKey, t)) : t.user = Ur();
			}
			...
			let n = (t.expires_at ?? Infinity) * 1e3 - Date.now() < kn;
			if (this._debug(e, `session has${n ? "" : " not"} expired with margin of ${kn}s`), n) {
	...
	async _callRefreshToken(e) {
		...
		try {
			...
			let t = await I(this.storage, this.storageKey), { data: n, error: i } = await this._refreshAccessToken(e);
			if (i) throw i;
			...
			if (await this._saveSession(n.session), this._sessionRemovalEpoch !== o) {
				this._debug(r, "commit guard (post-save): _removeSession ran during _saveSession, undoing write"), await L(this.storage, this.storageKey), this.userStorage && await L(this.userStorage, this.storageKey + "-user");
				let e = {
					data: null,
					error: new Xn()
				};
			...
			if (this._debug(r, "error", i), k(i)) {
				...
				if (!Yn(i)) {
					let e = await I(this.storage, this.storageKey);
	...
	async _saveSession(e) {
		...
		if (this.userStorage) {
			!n && t.user && await F(this.userStorage, this.storageKey + "-user", { user: t.user });
			let e = Object.assign({}, t);
			delete e.user;
			let r = Gr(e);
			await F(this.storage, this.storageKey, r);
		} else {
			let e = Gr(t);
	...
	async _removeSession() {
		this._sessionRemovalEpoch += 1, this._debug("#_removeSession()"), this.lastRefreshFailure = null, this.suppressGetSessionWarning = !1, await L(this.storage, this.storageKey), await Pr(this.storage, this.storageKey), await L(this.storage, this.storageKey + "-user"), this.userStorage && await L(this.userStorage, this.storageKey + "-user"), await this._notifyAllSubscribers("SIGNED_OUT", null);
	}
	...
	async _startAutoRefresh() {
		...
		let e = setInterval(() => this._autoRefreshTokenTick(), On);
					...
					try {
						...
						try {
							return await this._useSession(async (t) => {
								...
								let r = Math.floor((n.expires_at * 1e3 - e) / On);
								this._debug("#_autoRefreshTokenTick()", `access token expires in ${r} ticks, a tick lasts ${On}ms, refresh threshold is 3 ticks`), r <= 3 && await this._callRefreshToken(n.refresh_token);
							});
						} catch (e) {
	...
	async _enroll(e) {
		try {
			return await this._useSession(async (t) => {
				...
				}, e.factorType === "phone" ? { phone: e.phone } : e.factorType === "totp" ? { issuer: e.issuer } : {}), { data: a, error: o } = await V(this.fetch, "POST", `${this.url}/factors`, {
					body: i,
					headers: this.headers,
					jwt: n?.session?.access_token
				});
		...
		let t = async () => {
			try {
				return await this._useSession(async (t) => {
					...
					let i = await V(this.fetch, "POST", `${this.url}/factors/${e.factorId}/challenge`, {
						body: e,
						headers: this.headers,
						jwt: n?.session?.access_token
					});
	...
	async _getRecoveryCodesStatus() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await V(this.fetch, "GET", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
	...
	async _generateRecoveryCodes(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await V(this.fetch, "POST", `${this.url}/factors/recovery-codes`, {
		...
		let t = async () => {
			try {
				return await this._useSession(async (t) => {
					...
					let { data: i, error: a } = await V(this.fetch, "POST", `${this.url}/factors/recovery-codes/verify`, {
						body: { code: e.code },
						headers: this.headers,
						jwt: n?.session?.access_token
					});
					...
					let o = Object.assign({ expires_at: dr(i.expires_in) }, i);
	...
	async _regenerateRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await V(this.fetch, "POST", `${this.url}/factors/recovery-codes/regenerate`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
	...
	async _unenrollRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await V(this.fetch, "DELETE", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
		...
		try {
			return await this._useSession(async (t) => {
				...
				return r ? this._returnResult({
					data: null,
					error: r
				}) : n ? await V(this.fetch, "GET", `${this.url}/oauth/authorizations/${e}`, {
					headers: this.headers,
					jwt: n.access_token,
					xform: (e) => ({
						data: e,
	...
	async _approveAuthorization(e, t) {
		try {
			return await this._useSession(async (n) => {
				...
				let a = await V(this.fetch, "POST", `${this.url}/oauth/authorizations/${e}/consent`, {
					headers: this.headers,
					jwt: r.access_token,
					body: { action: "approve" },
					xform: (e) => ({
		...
		try {
			return await this._useSession(async (e) => {
				...
				return n ? this._returnResult({
					data: null,
					error: n
				}) : t ? await V(this.fetch, "GET", `${this.url}/user/oauth/grants`, {
					headers: this.headers,
					jwt: t.access_token,
					xform: (e) => ({
						data: e,
		...
		try {
			return await this._useSession(async (t) => {
				...
				return r ? this._returnResult({
					data: null,
					error: r
				}) : n ? (await V(this.fetch, "DELETE", `${this.url}/user/oauth/grants`, {
					headers: this.headers,
					jwt: n.access_token,
	...
	async fetchJwk(e, t = { keys: [] }) {
		...
		let { data: i, error: a } = await V(this.fetch, "GET", `${this.url}/.well-known/jwks.json`, { headers: this.headers });
		if (a) throw a;
	...
	async _startPasskeyRegistration() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await V(this.fetch, "POST", `${this.url}/passkeys/registration/options`, {
					headers: this.headers,
					jwt: t.access_token,
					body: {}
				});
	...
	async _verifyPasskeyRegistration(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await V(this.fetch, "POST", `${this.url}/passkeys/registration/verify`, {
					headers: this.headers,
					jwt: n.access_token,
					body: {
	...
	async _startPasskeyAuthentication(e) {
		B(this.experimental);
		try {
			let { data: t, error: n } = await V(this.fetch, "POST", `${this.url}/passkeys/authentication/options`, {
				headers: this.headers,
	...
	async _verifyPasskeyAuthentication(e) {
		B(this.experimental);
		try {
			let { data: t, error: n } = await V(this.fetch, "POST", `${this.url}/passkeys/authentication/verify`, {
				headers: this.headers,
				body: {
					challenge_id: e.challengeId,
					credential: e.credential
	...
	async _listPasskeys() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await V(this.fetch, "GET", `${this.url}/passkeys`, {
					headers: this.headers,
					jwt: t.access_token,
					xform: (e) => ({
						data: e,
	...
	async _updatePasskey(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await V(this.fetch, "PATCH", `${this.url}/passkeys/${e.passkeyId}`, {
					headers: this.headers,
					jwt: n.access_token,
	...
	async _deletePasskey(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { error: i } = await V(this.fetch, "DELETE", `${this.url}/passkeys/${e.passkeyId}`, {
					headers: this.headers,
					jwt: n.access_token,
					noResolveJson: !0
				});
...
function ca(e, n, r) {
	...
	if (!Gi(typeof e == "string" || e instanceof URL ? e : e.url, n)) return null;
	let a = i();
	if (!a || !a.traceparent) {
		var o;
...
function pa(e, t) {
	let { db: n, auth: r, realtime: i, global: a } = e, { db: o, auth: s, realtime: c, global: l } = t, u = la(e.tracePropagation), d = la(t.tracePropagation), f = {
		...
		realtime: G(G({}, c), i),
		storage: {},
		global: G(G(G({}, l), a), {}, { headers: G(G({}, l?.headers ?? {}), a?.headers ?? {}) }),
		tracePropagation: {
...
var ha = class extends Ni {
	...
	constructor(e, t, n) {
		...
		this.settings = o, this.storageKey = o.auth.storageKey ?? "", this.headers = o.global.headers ?? {}, o.accessToken ? (this.accessToken = o.accessToken, this.auth = new Proxy({}, { get: (e, t) => {
			...
			retry: o.db.retry
		}), this.storage = new En(this.storageUrl.href, this.headers, this.fetch, n?.storage), o.accessToken || this._listenForAuthEvents();
	}
	get functions() {
	...
	_initRealtimeClient(e) {
		return new Dt(this.realtimeUrl.href, G(G({}, e), {}, { params: G(G({}, { apikey: this.supabaseKey }), e?.params) }));
	}
...
var ya = "https://jnmdirvidffzxukbdmij.supabase.co"?.trim(), ba = "sb_publishable_ZOdMOklo0mbfEscPeRy6QA_SRLcssxc"?.trim(), xa = "harvest-tycoon:pending-account", Sa = !!(ya && ba), K = Sa ? _a(ya, ba, { auth: {
	persistSession: !0,
	autoRefreshToken: !0,
	detectSessionInUrl: !0,
	storageKey: "harvest-tycoon:auth"
} }) : null, q = null, J = null, Ca, Y = () => ({
	player: q,
	profile: J
});
function wa(e) {
	return typeof e == "string" && /^[A-Za-z0-9][A-Za-z0-9 _-]{2,19}$/.test(e.trim());
}
function Ta(e) {
	return e?.code === "23505" ? "That player name is already taken. Please choose another." : e?.code === "invalid_credentials" ? "The email address or password is incorrect." : e?.code === "email_not_confirmed" ? "Please confirm your email address using the link in your inbox." : e?.code === "42P01" || e?.code === "PGRST205" ? "The leaderboard is not available yet. You can keep farming." : e?.message || "Something went wrong. Check your connection and try again.";
}
function Ea(e, t) {
	try {
		localStorage.setItem(xa, JSON.stringify({
			username: e.trim(),
			stats: t
		}));
	} catch {}
...
function Da() {
	try {
		let e = JSON.parse(localStorage.getItem(xa) || "null");
		return localStorage.removeItem(xa), e;
	} catch {
		return null;
	}
}
async function Oa(e) {
	let t = await K.from("player_stats").select("player_id,username,currency,level,updated_at").eq("player_id", e.id).maybeSingle();
...
async function ka() {
	...
	if (Ca) return Ca;
	Ca = (async () => {
		let e = await K.auth.getSession();
		if (e.error) throw e.error;
		let t = e.data.session?.user;
		return !t || t.is_anonymous ? {
			player: null,
			profile: null
		} : Oa(t);
	})();
...
async function Aa(e, t) {
	let n = await K.auth.signInWithPassword({
		email: e.trim(),
		password: t
	});
	if (n.error) throw Error(Ta(n.error));
	return q = null, J = null, Oa(n.data.user);
}
async function ja(e, t, n, r) {
	if (!wa(n)) throw Error("Use 3–20 letters, numbers, spaces, underscores, or hyphens.");
	let i = (await K.auth.getSession()).data.session?.user, a = {
		email: e.trim(),
		password: t,
		options: { emailRedirectTo: new URL("/play.html", location.origin).href }
	}, o = i?.is_anonymous ? await K.auth.updateUser(a) : await K.auth.signUp({
		...a,
		options: {
			...a.options,
			data: { username: n.trim() }
		}
	});
	if (o.error) throw Error(Ta(o.error));
	return Ea(n, r), o.data.session ? (q = null, J = null, await Oa(o.data.user), await Pa(n, r), Da(), { confirmationRequired: !1 }) : { confirmationRequired: !0 };
}
async function Ma(e) {
	let t = Da();
	if (!t) return null;
	try {
		return await Pa(t.username, t.stats ?? e);
	} catch (n) {
		throw Ea(t.username, t.stats ?? e), n;
	}
}
async function Na() {
	if (!K) return;
	let e = await K.auth.signOut();
	if (e.error) throw Error(Ta(e.error));
	q = null, J = null;
}
async function Pa(e, t) {
	if (!wa(e)) throw Error("Use 3–20 letters, numbers, spaces, underscores, or hyphens.");
	let { player: n, profile: r } = await ka();
	if (!n) throw Error("Sign in first to save your progress.");
	let i = await (r ? K.from("player_stats").update({ username: e.trim() }).eq("player_id", n.id) : K.from("player_stats").upsert({
		player_id: n.id,
		username: e.trim(),
		currency: t.currency,
		level: t.level
	}, { onConflict: "player_id" })).select("player_id,username,currency,level,updated_at").single();
	if (i.error) throw Error(Ta(i.error));
	return J = i.data, J;
}
function Fa(e) {
	K && K.auth.onAuthStateChange((t, n) => {
		t === "INITIAL_SESSION" || t === "TOKEN_REFRESHED" || setTimeout(() => {
			q = null, J = null, e(t, n);
		}, 0);
	});
}
...
//#region src/sync.js
function Ia(e, t, n) {
	let r = (e, t) => Math.min(2147483647, Math.max(t, Math.floor(Number.isFinite(e) ? e : t)));
	return {
		player_id: e,
		username: t,
		currency: r(n.currency, 0),
		level: r(n.level, 1)
	};
}
function La({ getIdentity: e, write: t, onStatus: n, interval: r = 4e3, now: i = () => Date.now(), schedule: a = setTimeout, cancel: o = clearTimeout }) {
	let s = null, c = !1, l = !1, u = -Infinity, d;
	function f(e) {
		s = {
			currency: e.currency,
			level: e.level
	...
	async function m() {
		if (l || !c || !s) return !1;
		let { player: a, profile: o } = e();
		if (!a || !o) return !1;
		if (i() - u < r) return p(), !1;
		let d = s, f = Ia(a.id, o.username, d);
		l = !0, c = !1, u = i(), n("syncing");
		let m = !1;
		try {
			await t(f), m = !0, n(c ? "pending" : "synced");
		} catch {
			c = !0, n("offline");
		} finally {
			l = !1, m && c && p();
...
function za(e, { rows: t, own: n, rank: r }, i) {
...
function Ba({ onOpen: e, onName: t, onRetry: n, onSignIn: r, onRegister: i, onSignOut: a }) {
	...
	return document.querySelectorAll("[data-cloud-close]").forEach((e) => e.onclick = () => e.closest("dialog").close()), document.querySelectorAll("[data-auth-tab]").forEach((e) => e.onclick = () => u(e.dataset.authTab)), o.onclick = () => {
		...
		try {
			await t(X("username-input").value), l("leaderboard-dialog"), await e();
		} catch (e) {
		...
		promptName: d,
		requireAuth() {
			X("auth-dialog").open || l("auth-dialog");
		},
		authenticated() {
			X("auth-dialog").open && X("auth-dialog").close();
		},
		configurationError() {
			l("auth-dialog"), X("auth-message").textContent = "The account connection has not been configured yet.";
		},
		authMessage(e) {
...
//#endregion
//#region src/main.js
var Va = "harvest-tycoon:pending-stats", Z = {
	currency: 180,
	level: 1
}, Ha = !1, Ua = !1, Wa = null, Ga = 0, Ka = null, Q = La({
	getIdentity: Y,
	write: async (e) => {
		let { error: t } = await K.from("player_stats").upsert(e, { onConflict: "player_id" });
		if (t) throw t;
	},
	onStatus: (e) => {
		if ($.status({
			...
			offline: "Offline · we will try again"
		}[e]), e === "synced") try {
	...
	onOpen: Xa,
	onRetry: async () => (await Ja(), Q.retry(), Xa()),
	onName: async (e) => {
		await Pa(e, Z), $.setProfile(Y().profile, Y().player), Q.queue(Z);
	},
	onSignIn: async (e, t) => {
		await Aa(e, t), await Ja(!0);
	},
	onRegister: async (e, t, n) => ja(e, t, n, Z),
	onSignOut: async () => {
		await Na(), Ka = null, $.requireAuth();
	}
});
function qa() {
	let { player: e, profile: t } = Y();
	!e || !t || Ka === e.id || (Ka = e.id, window.harvestAccountProgress = {
		currency: t.currency,
		level: t.level
...
async function Ja(e = !1) {
	...
	Wa = (async () => {
		try {
			if (await ka(), !Y().player) {
				$.requireAuth();
				return;
			}
			await Ma(Z), $.authenticated(), $.setProfile(Y().profile, Y().player), Y().profile ? ($.status("Connected to your account"), qa()) : ($.status("Choose your player name"), Ya()), Q.retry();
		} catch (e) {
			$.status("Connection failed"), $.authMessage(Ta(e));
		}
	})();
	try {
		return await Wa;
...
function Ya() {
	Ha && Y().player && !Y().profile && !Ua && (Ua = !0, $.promptName());
}
async function Xa() {
	if (!Sa) return;
	let e = ++Ga;
	$.message("Gathering the latest scores…");
	try {
		if (await Ja(), !Y().player) return;
		if (!Y().profile) {
			$.promptName();
			return;
		}
		await Q.flush();
		let t = await Ra(K, Y().player.id);
		e === Ga && za($.results, t, Y().player.id);
	} catch (t) {
		e === Ga && $.message(Ta(t));
	}
}
function Za(e) {
	...
	try {
		localStorage.getItem(Va) && Q.queue(Z);
	} catch {}
}
window.addEventListener("farm:ready", (e) => Za(e.detail)), window.addEventListener("farm:stats", (e) => {
	Z = {
		currency: e.detail.currency,
		level: e.detail.level
	};
	try {
		localStorage.setItem(Va, JSON.stringify(Z));
	} catch {}
	Sa && Y().player && Q.queue(Z);
}), window.harvestStats && Za(window.harvestStats()), window.addEventListener("online", () => {
	Ja(), Q.retry();
}), Fa(() => {
	Ua = !1, Ja(!0).then(() => {
		$.open && Xa();
